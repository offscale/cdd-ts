...
The integration tests for this toolchain currently fail when running natively against the Stoplight Prism mock server (`./local-test.sh only-test` with `ONLY_TEST=cdd-ts`).

## Instructions
1. Run the test locally for this specific language using: 
   `cd .. && ONLY_TEST=cdd-ts ./local-test.sh only-test`
...
4. Verify your changes by ensuring `cd .. && ONLY_TEST=cdd-ts ./local-test.sh only-test` passes successfully (including the coverage validation and chaos saboteur audits).
...
The TypeScript SDK fails with `500 Server Error` (e.g. `should call createUsersWithArrayInput successfully`). The Java server likely crashed attempting to parse the TypeScript client's JSON payload. Ensure the payload strictly matches the expected array schema.
