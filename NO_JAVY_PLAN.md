...
_Goal: Transform the Go CLI into a C-ABI compliant shared WASM library._
...
_Goal: Rewrite `ts-morph` from a JS wrapper into an AssemblyScript memory-safe bridge._
...
_Goal: Ensure the core application complies with AssemblyScript constraints and WASI._

- [x] **WASI File I/O:** Replace all Node `fs` and `path` calls with `@assemblyscript/wasi-shim` equivalents.
    - [x] _Edge case:_ Handle WASI directory pre-opens (`--dir=.`) dynamically.
...
- [x] **Static Linking Pipeline:**
    - [x] If _not_ using Component Model, configure `wasm-ld` to merge the Go `.wasm` and the AS `.wasm`.
...
- [x] **Memory Profiling:**
    ...
    - [x] _Edge case:_ Test against massive OpenAPI specs (e.g., `stripe.json`, >5MB) to ensure the FFI bridge doesn't cause out-of-memory (OOM) errors in the 32-bit WASM address space.
