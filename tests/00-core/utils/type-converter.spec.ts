...
describe("Core Utils: Type Converter", () => {
	...
	const configWithDate: GeneratorConfig = {
		...config,
		options: { ...config.options, dateType: "Date" },
	};
	...
	describe("getTypeScriptType", () => {
		describe("dependentSchemas and dependentRequired", () => {
			it("should handle basic dependentSchemas", () => {
				...
				expect(result).toContain(
					"({ foo: string | number | boolean | object | undefined | null }",
				);
			...
			it("should handle basic dependentRequired", () => {
				...
				expect(result).toContain(
					"({ 'invalid-key': string | number | boolean | object | undefined | null } & { 'q-u-x': string | number | boolean | object | undefined | null })",
				);
			});
		...
		it("should resolve $dynamicRef to a known type", () => {
			...
			expect(utils.getTypeScriptType(schema, config, knownTypes)).toBe(
				"DynamicUser",
			);
		});
		...
		it("should handle anyOf compositions", () => {
			const schema: SwaggerDefinition = {
				anyOf: [{ type: "boolean" }, { $ref: "#/components/schemas/User" }],
			};
			expect(utils.getTypeScriptType(schema, config, ["User"])).toBe(
				"boolean | User",
			);
		...
		it("should use named enum title if present in knownTypes", () => {
			...
			expect(utils.getTypeScriptType(schema, config, ["MyEnum"])).toBe(
				"MyEnum",
			);
		});
		...
		it('should generate a union type for enum if style is "union"', () => {
			const conf = {
				...config,
				options: { ...config.options, enumStyle: "union" as const },
			};
		...
		it("should fall through to default string if contentMediaType is string but not textual or binary", () => {
			const schema: SwaggerDefinition = {
				type: "string",
				contentMediaType: "application/something-weird",
			};
			expect(utils.getTypeScriptType(schema, config, [])).toBe("string");
		});
		...
		it("should return string if format is date/date-time and config is string", () => {
			expect(
				utils.getTypeScriptType({ type: "string", format: "date" }, config),
			).toBe("string");
		});

		it("should return Blob for binary string format", () => {
			expect(
				utils.getTypeScriptType({ type: "string", format: "binary" }, config),
			).toBe("Blob");
		});
		...
		it("should use configured int64Type", () => {
			const conf = {
				...config,
				options: { ...config.options, int64Type: "string" as const },
			};
		...
		it("should use unevaluatedItems as rest type for prefixItems tuples", () => {
			...
			expect(utils.getTypeScriptType(schemaTrue, config, [])).toBe(
		...
		it("should merge patternProperties with explicit properties", () => {
			...
			expect(res).toContain(
				"[key: string]: number | string | number | boolean | object | undefined | null",
			);
		});
		...
		it("should handle boolean type", () => {
			expect(utils.getTypeScriptType({ type: "boolean" }, config)).toBe(
				"boolean",
			);
		});
	...
	describe("getRequestBodyType", () => {
		...
		it("should prioritize JSON content types", () => {
			...
			expect(
				utils.getRequestBodyType(
					rb as string | number | boolean | object | undefined | null,
					config,
					[],
				),
			).toBe("number");
	...
	describe("getResponseType", () => {
		...
		it("should return schema type for application/json", () => {
			...
			expect(
				utils.getResponseType(
					resp as string | number | boolean | object | undefined | null,
					config,
					[],
				),
			).toBe("boolean");
	...
	describe("isDataTypeInterface", () => {
		...
		it("should return false for comments in type string", () => {
			expect(utils.isDataTypeInterface("string /* JSON: number */")).toBe(
				false,
			);
		});
