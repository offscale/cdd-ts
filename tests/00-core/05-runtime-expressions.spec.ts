...
describe("Core: Runtime Expression Evaluator", () => {
	...
	describe("Expression Resolution", () => {
		it("should resolve $url", () => {
			expect(evaluateRuntimeExpression("$url", mockContext)).toBe(
		...
		it("should pass through constant strings", () => {
			expect(evaluateRuntimeExpression("hello world", mockContext)).toBe(
				"hello world",
			);
		});
		...
		describe("$request sources", () => {
			...
			it("should return first header value when header is an array", () => {
				expect(
					evaluateRuntimeExpression("$request.header.Accept", mockContext),
				).toBe("application/json");
			});

			it("should return undefined for missing request header", () => {
				expect(
					evaluateRuntimeExpression("$request.header.Missing", mockContext),
				).toBeUndefined();
			});

			it("should resolve request query (case-sensitive)", () => {
				expect(
					evaluateRuntimeExpression("$request.query.search", mockContext),
				).toBe("foo");
				expect(
					evaluateRuntimeExpression("$request.query.Search", mockContext),
				).toBeUndefined();
			...
			it("should resolve first query value when array is provided", () => {
				expect(
					evaluateRuntimeExpression("$request.query.tags", mockContext),
				).toBe("a");
			});

			it("should resolve request path (case-sensitive)", () => {
				expect(evaluateRuntimeExpression("$request.path.id", mockContext)).toBe(
					"123",
				);
			});

			it("should resolve full request body", () => {
				expect(evaluateRuntimeExpression("$request.body", mockContext)).toEqual(
			...
			it("should resolve request body via pointer", () => {
				expect(
					evaluateRuntimeExpression("$request.body#/user/id", mockContext),
				).toBe(999);
				expect(
					evaluateRuntimeExpression("$request.body#/user/roles/0", mockContext),
				).toBe("admin");
			});

			it("should return undefined for malformed request body expressions", () => {
				expect(
					evaluateRuntimeExpression("$request.bodyFoo", mockContext),
				).toBeUndefined();
			});

			it("should return undefined for unknown request expressions", () => {
				expect(
					evaluateRuntimeExpression("$request.unknown", mockContext),
				).toBeUndefined();
			});
		});
		...
		describe("$response sources", () => {
			it("should resolve response header", () => {
				expect(
					evaluateRuntimeExpression("$response.header.Location", mockContext),
				).toBe("https://example.com/users/999");
			});

			it("should resolve full response body", () => {
				expect(
					evaluateRuntimeExpression("$response.body", mockContext),
				).toEqual(mockContext.response?.body);
			});

			it("should resolve response body via pointer", () => {
				expect(
					evaluateRuntimeExpression("$response.body#/id", mockContext),
				).toBe(999);
				expect(
					evaluateRuntimeExpression(
			...
			it("should return undefined if response is missing", () => {
				...
				expect(
					evaluateRuntimeExpression("$response.body", noRes),
				).toBeUndefined();
			});
			...
			it("should return undefined for malformed body expressions", () => {
				...
				expect(
					evaluateRuntimeExpression("$response.bodyFoo", mockContext),
				).toBeUndefined();
			});

			it("should return undefined for unknown response expressions", () => {
				expect(
					evaluateRuntimeExpression("$response.unknown", mockContext),
				).toBeUndefined();
			});
		});
		...
		describe("String Interpolation", () => {
			it("should interpolate simple values", () => {
				expect(
					evaluateRuntimeExpression("Status: {$statusCode}", mockContext),
				).toBe("Status: 201");
			});

			it("should interpolate multiple values", () => {
				const expr =
					"https://api.com/items/{$response.body#/id}?token={$request.header.x-request-id}";
				expect(evaluateRuntimeExpression(expr, mockContext)).toBe(
					"https://api.com/items/999?token=req-1",
				);
			});
