...
describe("Emitter: OAuthHelperGenerator", () => {
	...
	it("should generate service and component if Authorization Code flow is present", () => {
		...
		expect(
			project.getSourceFile(
				"/out/auth/oauth-redirect/oauth-redirect.component.ts",
			),
		).toBeDefined();
		expect(
			project
				.getFileSystem()
				.readFileSync("/out/auth/oauth-redirect/oauth-redirect.component.html"),
		).toBe("<p>Redirecting...</p>");
	...
	it("should generate Password flow methods if password flow is defined", () => {
		...
		const sourceFile = project.getSourceFileOrThrow(
			"/out/auth/oauth.service.ts",
		);
		const service = sourceFile.getClassOrThrow("OAuthHelperService");

		expect(service.getProperty("tokenUrl")?.getInitializer()?.getText()).toBe(
	...
	it("should generate Device Authorization flow logic", () => {
		...
		expect(
			service
				.getProperty("deviceAuthorizationUrl")
				?.getInitializer()
				?.getText(),
		).toBe("'https://device.example.com/device'");
	...
	it("should generate Implicit flow logic", () => {
			...
			components: {
				securitySchemes: {
					Imp: {
						...
						flows: {
							implicit: {
								authorizationUrl: "https://imp.com/auth",
								scopes: {},
							},
						},
					},
		...
		expect(
			service.getProperty("authorizationUrl")?.getInitializer()?.getText(),
		).toBe("'https://imp.com/auth'");
			...
			components: {
				securitySchemes: {
					Implicit1: {
						...
						flows: {
							implicit: {
								authorizationUrl: "https://first.example.com/auth",
								scopes: {},
							},
						},
					},
					...
					ClientCreds4: {
						...
						flows: {
							clientCredentials: {
								tokenUrl: "https://fourth.example.com/token",
								scopes: {},
							},
						},
					},
					Password3: {
						...
						flows: {
							password: {
								tokenUrl: "https://third.example.com/token",
								scopes: {},
							},
						},
					},
