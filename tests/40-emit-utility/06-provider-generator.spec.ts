...
describe("Emitter: ProviderGenerator", () => {
	const runGenerator = (
		...
		new TokenGenerator(project, fullConfig.clientName).generate(
			fullConfig.output,
		);
		new BaseInterceptorGenerator(project, fullConfig.clientName).generate(
			fullConfig.output,
		);
		...
		if (Object.keys(parser.getSecuritySchemes()).length > 0) {
			...
			const authInterceptorResult = new AuthInterceptorGenerator(
				parser,
				project,
			).generate(fullConfig.output);
			tokenNames = authInterceptorResult?.tokenNames || [];
		}

		new ProviderGenerator(parser, project, tokenNames).generate(
			fullConfig.output,
		);
	...
	it('should use "Default" client name when none is provided in config', () => {
		...
		expect(fileContent).toContain(
			"export function provideDefaultClient(config: DefaultConfig)",
		);
	...
	it("should not generate if generateServices is false", () => {
		const fileContent = runGenerator(emptySpec, {
			options: { generateServices: false } as
				| string
				| number
				| boolean
				| object
	...
	it("should generate a basic provider without security or date transform", () => {
		...
		expect(fileContent).toContain(
			"export function provideTestClient(config: TestConfig)",
		);
	...
	it("should add providers for mutualTLS", () => {
		...
		expect(fileContent).toContain(
			"httpsAgentConfig?: Record<string, string | number | boolean | object | undefined | null>",
		);
	...
	it("should generate an empty custom interceptors array if none are provided", () => {
		...
		expect(fileContent).toContain(
			`provide: HTTP_INTERCEPTORS_TEST, useValue: customInterceptors`,
		);
	});
	...
	it("should use default tokenNames when omitted", () => {
		...
		const fileContent = project
			.getSourceFileOrThrow("/out/providers.ts")
			.getText();
