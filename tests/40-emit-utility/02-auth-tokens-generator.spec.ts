...
describe("Emitter: AuthTokensGenerator", () => {
	it("should generate a file with API_KEY_TOKEN and BEARER_TOKEN_TOKEN", () => {
		...
		const fileContent = project
			.getSourceFileOrThrow("/out/auth/auth.tokens.ts")
			.getText();
		...
		expect(fileContent).toContain(
			"new InjectionToken<string | (() => string)>('BEARER_TOKEN')",
		);
	});
	...
	it("should generate SECURITY_CONTEXT_TOKEN", () => {
		...
		expect(fileContent).toContain(
			"new HttpContextToken<Record<string, string[]>[]>(() => [])",
		);
	});
	...
	it("should generate HTTPS_AGENT_CONFIG_TOKEN and HTTPS_AGENT_CONTEXT_TOKEN", () => {
		...
		expect(fileContent).toContain(
			"new InjectionToken<Record<string, string | number | boolean | object | undefined | null>>('HTTPS_AGENT_CONFIG')",
		);
		expect(fileContent).toContain("export const HTTPS_AGENT_CONTEXT_TOKEN");
		expect(fileContent).toContain(
			"new HttpContextToken<Record<string, string | number | boolean | object | undefined | null> | null>(() => null)",
		);
	});
});
