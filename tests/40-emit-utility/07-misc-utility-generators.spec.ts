...
describe("Emitter: Miscellaneous Utility Generators", () => {
	it("AuthHelperGenerator should generate a complete service", () => {
		...
		const fileContent = project
			.getSourceFileOrThrow("/out/auth/auth-helper.service.ts")
			.getText();
	...
	it("DateTransformerGenerator should generate interceptor and helpers", () => {
		...
		const fileContent = project
			.getSourceFileOrThrow("/out/utils/date-transformer.ts")
			.getText();
	...
	it("FileDownloadGenerator should generate all download helpers", () => {
		...
		const fileContent = project
			.getSourceFileOrThrow("/out/utils/file-download.ts")
			.getText();
		...
		expect(fileContent).toContain(
			"export function downloadFileOperator<T extends Blob | HttpResponse<Blob>>",
		);
		expect(fileContent).toContain(
