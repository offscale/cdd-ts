...
describe("Emitter: InfoGenerator", () => {
	...
	it("should export API tags including summary (OAS 3.x)", () => {
		const spec = {
			...
			tags: [
				...
				{
					name: "Users",
					externalDocs: { url: "https://users.doc", description: "User Guide" },
				},
			],
			paths: {},
		};
	...
	it("should preserve x- extensions and allow them in metadata types", () => {
		...
		const { API_INFO, API_TAGS, API_EXTERNAL_DOCS } =
			compileGeneratedFile(project);
		...
		const sourceFile = project.getSourceFileOrThrow("/out/info.ts");
		expect(
			sourceFile.getInterfaceOrThrow("ApiInfo").getIndexSignatures().length,
		).toBe(1);
		expect(
			sourceFile.getInterfaceOrThrow("ApiTag").getIndexSignatures().length,
		).toBe(1);
		expect(
			sourceFile.getInterfaceOrThrow("ApiContact").getIndexSignatures().length,
		).toBe(1);
		expect(
			sourceFile.getInterfaceOrThrow("ApiLicense").getIndexSignatures().length,
		).toBe(1);
		expect(
			sourceFile.getInterfaceOrThrow("ApiExternalDocs").getIndexSignatures()
				.length,
		).toBe(1);
	});
