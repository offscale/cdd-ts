...
function getXmlBuilder() {
	...
	return (
		moduleScope.exports as string | number | boolean | object | undefined | null
	).XmlBuilder;
}
...
describe("Utility: XmlBuilder", () => {
	...
	describe("Standard (Legacy) Behavior", () => {
		it("should serialize simple primitives", () => {
			...
			expect(XmlBuilder.serialize(true, "active")).toBe(
				"<active>true</active>",
			);
		});
		...
		it("should default wrapped array item names to the parent name when none specified", () => {
			...
			expect(xml).toBe(
				"<Root><list><list>a</list><list>b</list></list></Root>",
			);
		});
	...
	describe("OAS 3.2 Null Handling (Appendix G)", () => {
		it('should serialize root null as xsi:nil="true"', () => {
			...
			expect(xml).toBe(
				'<Root xsi:nil="true" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" />',
			);
		});
		...
		it('should serialize null property as xsi:nil="true" element', () => {
			...
			expect(xml).toContain(
				'<empty xsi:nil="true" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" />',
			);
		});
	...
	describe("OAS 3.2 nodeType Support", () => {
		...
		it('should handle nodeType: "element" on arrays (Wrapping)', () => {
			...
			expect(xml).toBe(
				"<Root><list><item>1</item><item>2</item></list></Root>",
			);
		});
		...
		it('should handle nodeType: "none" (Unwrapping/Grouping)', () => {
			...
			expect(xml).toBe(
				"<Root><version>1.0</version><author>me</author></Root>",
			);
		});
		...
		it('should handle nodeType: "text" (Inner Text)', () => {
			...
			expect(xml).toBe(
				'<Element attr="attrVal">This is text content</Element>',
			);
		});
	...
	describe("Namespaces and Prefixes (OAS 3.2)", () => {
		it("should apply default namespace to root element", () => {
			...
			expect(xml).toBe(
				'<Root xmlns="http://example.com/schema"><id>1</id></Root>',
			);
		});
		...
		it("should apply prefix and namespace to root element", () => {
			...
			expect(xml).toBe(
				'<ex:Root xmlns:ex="http://example.com/schema"><id>1</id></ex:Root>',
			);
		});
		...
		it("should apply namespaces to wrapped array elements", () => {
			...
			expect(xml).toBe(
				'<Root><list:items xmlns:list="http://lists.com"><i>1</i><i>2</i></list:items></Root>',
			);
		});
	});
});
