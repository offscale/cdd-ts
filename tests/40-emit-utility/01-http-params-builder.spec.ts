...
class MockHttpParams {
	...
	constructor(options?: {
		...
		if (options?.fromObject) {
			Object.entries(
				options.fromObject as Record<string, string | number | boolean | null>,
			).forEach(([k, v]) => {
...
function getBuilderContext() {
	...
	const codeWithoutImports = sourceFile
		.getText()
		.replace(/import\s+.*from\s+['"].*['"];?/g, "");
	const encoderCodeWithoutImports = encoderFile
		.getText()
		.replace(/import\s+.*from\s+['"].*['"];?/g, "");
	...
	new Function("exports", "HttpParams", "ContentEncoder", jsCode)(
		exportsMock,
		MockHttpParams,
		ContentEncoder,
	);
	...
	return {
		Builder: (
			exportsMock as string | number | boolean | object | undefined | null
		).HttpParamsBuilder,
...
describe("Utility: HttpParamsBuilder", () => {
	...
	const createParams = () =>
		new MockHttpParams({ encoder: new ApiParameterCodec() });
	...
	describe("serializeQueryParam", () => {
		...
		it("should emit empty string if allowEmptyValue is true and value is null", () => {
			...
			const res = Builder.serializeQueryParam(
				params,
				{ name: "flag", allowEmptyValue: true },
				null,
			);
	...
	describe("serializePathParam", () => {
		it("should encode path delimiters when allowReserved=true", () => {
			const result = Builder.serializePathParam(
				"id",
				"a/b?c#d",
				"simple",
				false,
	...
	describe("serializeCookieParam (OAS 3.2 Strict Compliance)", () => {
		...
		it("should serialize Array with explode=true (Multiple cookies, same name)", () => {
			...
			const res = Builder.serializeCookieParam(
				"color",
				["blue", "black"],
				"form",
				true,
		...
		it('should handle style="cookie" by NOT percent-encoding values (OAS 3.2)', () => {
			...
			expect(Builder.serializeCookieParam("msg", val, "form", true)).toBe(
				"msg=hello%20world!",
			);
			// cookie style -> raw: hello world!

			expect(Builder.serializeCookieParam("msg", val, "cookie", true)).toBe(
				"msg=hello world!",
			);
		});
		...
		it("should allow reserved characters if allowReserved=true via encodeReserved", () => {
			...
			expect(
				Builder.serializeCookieParam("path", val, "form", true, true),
			).toBe("path=a/b+c");
			// standard form (allowReserved=false) -> percent encoded

			expect(
				Builder.serializeCookieParam("path", val, "form", true, false),
			).toBe("path=a%2Fb%2Bc");
		});
