...
describe("Generators (Angular): Service Generators (Coverage)", () => {
	const ensureResponses = (
		...
		for (const pathItem of Object.values(spec.paths)) {
			...
			for (const method of methods) {
				const operation = (
					pathItem as string | number | boolean | object | undefined | null
				)[method];
			...
			if (
				...
				for (const operation of Object.values(
					...
					if (
						operation &&
						(operation as string | number | boolean | object | undefined | null)
							.responses === undefined
					) {
						(
							operation as string | number | boolean | object | undefined | null
	...
	it("should import models for parameter types that are interfaces", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/paramIsRef.service.ts",
		);
		...
		expect(
			modelImport
				?.getNamedImports()
				.map((i: string | number | boolean | object | undefined | null) =>
					i.getName(),
				),
		).toContain("User");
	});
	...
	it("should not import string | number | boolean | object | undefined | null models if only primitive parameters are used", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/primitives.service.ts",
		);
		const _modelImport = serviceFile.getImportDeclaration(
			(imp) => imp.getModuleSpecifierValue() === "../models",
		);
	});
	...
	it("should generate methods for multipart/form-data", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/formData.service.ts",
		);
		...
		expect(methodBody).toContain(
			"MultipartBuilder.serialize(body, multipartConfig);",
		);
		expect(methodBody).toContain(
	...
	it("should generate methods for application/x-www-form-urlencoded", () => {
		const project = run(coverageSpecPart2);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/urlEncoded.service.ts",
		);
		const methodBody = serviceFile
	...
	it("should not import models for services that only return primitives", () => {
		const project = run(coverageSpecPart2);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/primitiveResponse.service.ts",
		);
		const modelImport = serviceFile.getImportDeclaration(
			(imp: ImportDeclaration) => imp.getModuleSpecifierValue() === "../models",
		);
	...
	it("should handle request body without a schema", () => {
		const project = run(branchCoverageSpec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/bodyNoSchema.service.ts",
		);
		const method = serviceFile
			.getClassOrThrow("BodyNoSchemaService")
			.getMethodOrThrow("postBodyNoSchema");
		...
		expect(param?.getType().getText()).toBe(
	...
	it("should handle operations with only required parameters", () => {
		const project = run(branchCoverageSpec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/allRequired.service.ts",
		);
		const method = serviceFile
			.getClassOrThrow("AllRequiredService")
			.getMethodOrThrow("getAllRequired");
		const overloads = method.getOverloads();

		const responseOverload = overloads.find(
			(o: string | number | boolean | object | undefined | null) =>
	...
	it('should fall back to "string | number | boolean | object | undefined | null" for responseType when no success response is defined', () => {
		const project = run(branchCoverageSpec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/noSuccessResponse.service.ts",
		);
		const method = serviceFile
			.getClassOrThrow("NoSuccessResponseService")
			.getMethodOrThrow("getNoSuccess");
	...
	it("should cover itemSchema and undefined schema in responses and parameters", () => {
		...
		op.responses!["200"].content!["application/json"].itemSchema = {
			type: "string",
		};
	...
	it("should handle default responses and responses without content", () => {
		...
		const project = run(spec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/defaultResponse.service.ts",
		);
		const modelImport = serviceFile.getImportDeclaration(
			(imp: ImportDeclaration) => imp.getModuleSpecifierValue() === "../models",
		);
		...
		const noContentServiceFile = project.getSourceFileOrThrow(
			"/out/services/noContentResponse.service.ts",
		);
	...
	it("should generate SSE parsing logic for text/event-stream", () => {
		...
		const project = run(spec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/sse.service.ts",
		);
		const method = serviceFile
			.getClassOrThrow("SseService")
			.getMethodOrThrow("getEvents");
