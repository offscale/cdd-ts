...
describe("Generators (Angular): ServiceGenerator", () => {
	...
	it("should handle methods with query, path, and body parameters", () => {
		...
		const method = serviceClass
			...
			.find(
				(m) =>
					m.getName() === "updateUser" &&
					m.getParameters().some((p) => p.getName() === "user"),
			)!;
		...
		expect(body).toContain(
			"return this.http.put<string | number | boolean | object | undefined | null>(url, user, requestOptions as object);",
		);
	...
	it("should use a custom method name when provided in config", () => {
		const project = createTestEnvironment(coverageSpec, {
			customizeMethodName: (opId: string) =>
				`custom_${opId.replace(/-/g, "_")}`,
		});
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/customName.service.ts",
		);
	...
	it("should fall back to path-based name if customizer exists but op has no ID", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/noOperationId.service.ts",
		);
	...
	it("should de-duplicate method names from conflicting customizer outputs", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/duplicateName.service.ts",
		);
	...
	it("should import a model used in a request body", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/users.service.ts",
		);
		const importDecl = serviceFile.getImportDeclaration(
			(d) => d.getModuleSpecifierValue() === "../models",
		)!;
	...
	it("should not add primitive array types to model imports", () => {
		const project = createTestEnvironment(coverageSpec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/stringArray.service.ts",
		);
		const importDecl = serviceFile.getImportDeclaration(
			(d) => d.getModuleSpecifierValue() === "../models",
		);
	...
	it("should add SECURITY_CONTEXT_TOKEN import only if security override is present AND global security exists", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/public.service.ts",
		);
		...
		expect(authImport?.getNamedImports().map((i) => i.getName())).toContain(
	...
	it("should import xml, content encoder/decoder, and extensions when needed", () => {
		...
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/mixed.service.ts",
		);

		expect(
			serviceFile.getImportDeclaration("../utils/xml-builder"),
		).toBeDefined();
		expect(
			serviceFile.getImportDeclaration("../utils/xml-parser"),
		).toBeDefined();
		expect(
			serviceFile.getImportDeclaration("../utils/content-decoder"),
		).toBeDefined();
		expect(
			serviceFile.getImportDeclaration("../utils/content-encoder"),
		).toBeDefined();
		expect(
			serviceFile.getImportDeclaration("../tokens/extensions.token"),
		).toBeDefined();
	});
	...
	it("should emit contentEncoderConfig for encoded query parameters", () => {
		...
		const project = createTestEnvironment(spec);
		const serviceFile = project.getSourceFileOrThrow(
			"/out/services/encoded.service.ts",
		);
		const method = serviceFile
			.getClassOrThrow("EncodedService")
			.getMethodOrThrow("getEncoded");
		const body = method.getBodyText() ?? "";
		...
		expect(normalizedBody).toContain(
			'"contentEncoderConfig":{"contentEncoding":"base64"}',
		);
		expect(normalizedBody).toContain(
