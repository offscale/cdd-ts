...
describe("Emitter: ServiceMethodGenerator (Edge Cases)", () => {
	...
	it("should warn and skip generation if operation has no methodName", () => {
		...
		const operationWithoutName: PathInfo = {
			path: "/test",
			method: "GET",
			operationId: "testOp",
		};
		...
		expect(
			serviceClass.getMethods().filter((m) => m.getName() === "testOp").length,
		).toBe(0);
		expect(warnSpy).toHaveBeenCalledWith(
			expect.stringContaining(
				"Skipping method generation for operation without a methodName",
			),
		);
		warnSpy.mockRestore();
	});
	...
	it("should NOT apply SECURITY_CONTEXT_TOKEN for explicit skip (default behavior)", () => {
		const { methodGen, serviceClass, parser } =
			createTestEnvironment(specEdgeTests);

		const op = parser.operations.find(
			(o: string | number | boolean | object | undefined | null) =>
				o.operationId === "getPublic",
		)!;
	...
	it("should generate context with SECURITY_CONTEXT_TOKEN when security scopes are present", () => {
		...
		const op = parser.operations.find(
			(o: string | number | boolean | object | undefined | null) =>
				o.operationId === "getOauthProtected",
		)!;
		op.methodName = "getOauthProtected";
		methodGen.addServiceMethod(serviceClass, op);

		const body = serviceClass
			.getMethodOrThrow("getOauthProtected")
			.getBodyText()!;
		expect(body).toContain(
			'.set(SECURITY_CONTEXT_TOKEN, [{"OAuth2":["read:admin"]}])',
		);
	});
	...
	it("should override basePath when operation servers are present", () => {
		...
		methodGen.addServiceMethod(serviceClass, op);
		const body = serviceClass
			.getMethodOrThrow("getWithServerOverride")
			.getBodyText()!;
