...
describe("Emitter: ServiceMethodGenerator (Response Precedence)", () => {
	...
	it("should prioritize specific 200 response over generic 2XX response", () => {
		const op: PathInfo = {
			...
			responses: {
				"2XX": {
					...
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/GenericModel" },
						},
					},
				},
				"200": {
					...
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/SpecificModel" },
						},
					},
				},
			},
		...
		const returnType = serviceClass
			.getMethodOrThrow("testSpecific")
			.getOverloads()[0]
			.getReturnType()
			.getText();
		...
		const op: PathInfo = {
			...
			responses: {
				default: {
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/DefaultModel" },
						},
					},
				},
