...
describe("Emitter: ServiceMethodGenerator (Content Negotiation)", () => {
	...
	it("should generate overloads for explicit Accept headers", () => {
		...
		const op = parser.operations.find(
			(o) => o.operationId === "getNegotiatedData",
		)!;
		...
		const xmlOverload = overloads.find((o) =>
			o.getText().includes("'Accept': 'application/xml'"),
		);
		...
		const jsonOverload = overloads.find((o) =>
			o.getText().includes("'Accept': 'application/json'"),
		);
		expect(jsonOverload).toBeDefined();

		// 3. Check Default overload
		const defaultOverload = overloads.find(
			(o) => !o.getText().includes("'Accept':"),
		);
	...
	it("should generate runtime logic to parse XML if Accept header requests it", () => {
		...
		const body = serviceClass
			.getMethodOrThrow("getNegotiatedData")
			.getBodyText()!;
	...
	it("should drop wildcard media types when more specific types exist", () => {
		...
		const op = parser.operations.find(
			(o) => o.operationId === "getWildcardData",
		)!;
		...
		expect(
			overloads.some((text) => text.includes("'Accept': 'application/json'")),
		).toBe(true);
		expect(
			overloads.some((text) => text.includes("'Accept': 'text/plain'")),
		).toBe(true);
		expect(overloads.some((text) => text.includes("'Accept': 'text/*'"))).toBe(
			false,
		);
		expect(overloads.some((text) => text.includes("'Accept': '*/*'"))).toBe(
			false,
		);
	});
});
