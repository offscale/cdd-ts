...
describe("Generators (Angular): ListComponentGenerator", () => {
	...
	describe("General Generation", () => {
		it("should handle API errors gracefully in generated code", () => {
			const listClass = project.getSourceFileOrThrow(
				"/admin/users/users-list/users-list.component.ts",
			);
		...
		it("should generate methods and html for custom actions", () => {
			const listClass = project
				.getSourceFileOrThrow(
					"/admin/servers/servers-list/servers-list.component.ts",
				)
			...
			const html = project
				.getFileSystem()
				.readFileSync(
					"/admin/servers/servers-list/servers-list.component.html",
				);
		...
		it('should fall back to the first property if no "id" is present', () => {
			const listClass = project
				.getSourceFileOrThrow(
					"/admin/events/events-list/events-list.component.ts",
				)
				.getClassOrThrow("EventsListComponent");
			const idProp = listClass
		...
		it("should handle a resource with only read-only properties", () => {
			...
			const resource = discoverAdminResources(parser).find(
				(r: Resource) => r.name === "readOnlyResource",
			)!;
			...
			expect(
				listClass.getProperty("idProperty")?.getInitializer()?.getText(),
			).toBe(`'id'`);
		});
	});
	...
	describe("Targeted Coverage Cases", () => {
		...
		it("should correctly map various action names to icons via abstract kinds", () => {
			const html = localProject
				.getFileSystem()
				.readFileSync(
					"/admin/iconTests/iconTests-list/iconTests-list.component.html",
				);
			...
			expect(startButtonHtml?.[1]).toContain(
				'<mat-icon aria-hidden="true">play_arrow</mat-icon>',
			);
		});
		...
		it('should generate an "id" column when a resource has no properties at all', () => {
			const listClass = localProject
				.getSourceFileOrThrow(
					"/admin/noPropsResource/noPropsResource-list/noPropsResource-list.component.ts",
				)
				.getClassOrThrow("NoPropsResourceListComponent");
			const idProp = listClass
				.getPropertyOrThrow("idProperty")
				.getInitializer()
				?.getText();
			...
			const html = localProject
				.getFileSystem()
				.readFileSync(
					"/admin/noPropsResource/noPropsResource-list/noPropsResource-list.component.html",
				);
		...
		it("should map kinds to icons via fallback switch cases", () => {
			...
			expect(
				...
				).mapKindToIcon("custom", "state-change"),
			).toBe("sync");

			expect(
				(
					generator as string | number | boolean | object | undefined | null
				).mapKindToIcon("custom", "navigation"),
			).toBe("arrow_forward");

			expect(
				(
					generator as string | number | boolean | object | undefined | null
				).mapKindToIcon("custom", "default"),
			).toBe("play_arrow");

			expect(
				(
					generator as string | number | boolean | object | undefined | null
				).mapKindToIcon("editItem", "default"),
			).toBe("edit");
		});
		...
		it("should skip onDelete when delete operation lacks methodName", () => {
			...
			const listClass = project
				.getSourceFileOrThrow(
					"/admin/orphans/orphans-list/orphans-list.component.ts",
				)
