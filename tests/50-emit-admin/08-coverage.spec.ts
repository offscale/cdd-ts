		...
		"/update-only/{id}": {
			put: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/UpdateOnly" },
						},
					},
				},
		...
		"/poly-mixed": {
			post: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/PolyMixed" },
						},
					},
				},
			...
			get: {
				...
				responses: {
					"200": {
						...
						content: {
							"application/json": {
								schema: { $ref: "#/components/schemas/NoSubmit" },
							},
						},
					},
				},
		...
		"/poly-primitive-only": {
			post: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { $ref: "#/components/schemas/PolyPrimitiveOnly" },
						},
					},
				},
				responses: { "201": { description: "ok" } },
...
describe("Admin Generators (Coverage)", () => {
	...
	it("resource-discovery should handle resources with no defined schemas", () => {
		...
		const resource = resources.find(
			(r: Resource) => r.name === "noSchemaResource",
		);
		expect(resource).toBeDefined();
		// It should fall back to a default 'id' property.
		expect(resource?.formProperties).toEqual([
	...
	it("resource-discovery should merge formData params and allOf schema properties", () => {
		...
		expect((fileProp?.schema as SwaggerDefinition).description).toBe(
			"Upload file",
		);
	...
	it("resource-discovery should handle operations without parameters or method names", () => {
		...
		(
			parser.operations as string | number | boolean | object | undefined | null
		).push({
			path: "",
			method: "",
			tags: [],
		...
		const defaultResource = resources.find(
			(r: Resource) => r.name === "default",
		);
		const op = defaultResource?.operations.find(
			(o) => o.path === "" && o.method === "",
		);
	...
	it("list-component-generator getIconForAction should fall back for string | number | boolean | object | undefined | null actions", () => {
		const getIconForAction = (action: string): string => {
			...
			if (lowerAction.includes("add") || lowerAction.includes("create"))
				return "add";
			if (lowerAction.includes("start") || lowerAction.includes("play"))
				return "play_arrow";
			if (lowerAction.includes("stop") || lowerAction.includes("pause"))
				return "pause";
			if (
				...
				return "refresh";
			if (lowerAction.includes("approve") || lowerAction.includes("check"))
				return "check";
			if (lowerAction.includes("cancel") || lowerAction.includes("block"))
				return "block";
			return "play_arrow";
		};
	...
	it("list-component-generator handles listable resource with no actions", () => {
		...
		const resource = discoverAdminResources(parser).find(
			(r: Resource) => r.name === "reports",
		)!;
		...
		const listClass = project
			.getSourceFileOrThrow(
				"/admin/reports/reports-list/reports-list.component.ts",
			)
			.getClassOrThrow("ReportsListComponent");
		const displayedColumns = listClass
			.getProperty("displayedColumns")
			?.getInitializer()
			?.getText() as string;
	...
	it("list-component-generator handles resource with no actions and non-id primary key", () => {
					...
					get: {
						...
						responses: {
							"200": {
								...
								content: {
									"application/json": {
										schema: { $ref: "#/components/schemas/DiagnosticInfo" },
									},
								},
							},
						},
		...
		const resource = discoverAdminResources(parser).find(
			(r: Resource) => r.name === "diagnostics",
		)!;
		const generator = new ListComponentGenerator(project);
		generator.generate(resource, "/admin");
		const listClass = project
			.getSourceFileOrThrow(
				"/admin/diagnostics/diagnostics-list/diagnostics-list.component.ts",
			)
...
describe("Admin: FormComponentGenerator (Coverage)", () => {
	...
	it("should generate update-only logic in onSubmit when no create op exists", () => {
		const formClass = project
			.getSourceFileOrThrow(
				"/admin/updateOnly/updateOnly-form/updateOnly-form.component.ts",
			)
		...
		expect(body).toContain(
			"const action$ = this.updateOnlyService.updateTheThing(this.id()!, finalPayload);",
		);
	...
	it("should handle polymorphic schemas with mixed primitive and object types", () => {
		const formClass = project
			.getSourceFileOrThrow(
				"/admin/polyMixed/polyMixed-form/polyMixed-form.component.ts",
			)
			.getClassOrThrow("PolyMixedFormComponent");

		const patchMethod = formClass.getMethod("patchForm");
		expect(patchMethod).toBeDefined();
		expect(patchMethod?.getBodyText()).toContain(
			"if (this.isType_SubObject(entity))",
		);
	...
	it("should not generate onSubmit for editable resource with no create/update ops", () => {
		const formClass = project
			.getSourceFileOrThrow(
				"/admin/noSubmit/noSubmit-form/noSubmit-form.component.ts",
			)
	...
	it("should not generate patchForm for simple forms", () => {
		const formClass = project
			.getSourceFileOrThrow(
				"/admin/simpleForm/simpleForm-form/simpleForm-form.component.ts",
			)
		...
		expect(ngOnInitMethod?.getBodyText()).toContain(
	...
	it("should generate an empty update method body for polymorphism with only primitives", () => {
		const formClass = project
			.getSourceFileOrThrow(
				"/admin/polyPrimitiveOnly/polyPrimitiveOnly-form/polyPrimitiveOnly-form.component.ts",
			)
	...
	it("should strip readOnly properties in getPayload", () => {
		...
		expect(body).toContain(
			"delete (payload as Record<string, string | number | boolean | object | undefined | null>)['id']",
		);
		expect(body).not.toContain(
			"delete (payload as Record<string, string | number | boolean | object | undefined | null>)['name']",
		);
	});
});
