...
describe("Admin: resource-discovery (Coverage)", () => {
	...
	const runDiscovery = (spec: object) => {
		const config: GeneratorConfig = { options: { admin: true } } as
			| string
			| number
			| boolean
			| object
	...
	it("should infer form properties from a 201 response when 200 is missing", () => {
		...
		expect(
			resource?.formProperties.some((p: FormProperty) => p.name === "name"),
		).toBe(true);
	});
	...
	it("should ignore formData parameters that are refs", () => {
		...
		expect(
			resource?.formProperties.some((p: FormProperty) => p.name === "good"),
		).toBe(true);
		// `bad` is skipped by the `!('$ref' in param.schema)` check.
		expect(
			resource?.formProperties.some((p: FormProperty) => p.name === "bad"),
		).toBe(false);
	});
	...
	it("should handle properties that are inline objects, not refs", () => {
		const resources = runDiscovery(branchCoverageSpec);
		const resource = resources.find(
			(r: Resource) => r.name === "inlineSchemaProperty",
		);
		expect(resource).toBeDefined();
		const prop = resource?.formProperties.find(
			(p: FormProperty) => p.name === "inline",
		);
		expect((prop?.schema as SwaggerDefinition).properties).toHaveProperty(
			"prop",
		);
	});
	...
	it("should collect properties from swagger 2.0 formData parameters", () => {
		...
		expect(resource).toBeDefined();
		expect(
			resource?.formProperties.some((p: FormProperty) => p.name === "file"),
		).toBe(true);
		expect(
			resource?.formProperties.some((p: FormProperty) => p.name === "metadata"),
		).toBe(true);
	});
					...
					get: {
						...
						responses: {
							"200": {
								description: "ok",
								content: {
									"application/json": { schema: { $ref: "#/non/existent" } },
								},
							},
						},
					},
	...
	it("should correctly identify a resource with PATCH as editable", () => {
		const resources = runDiscovery(branchCoverageSpec);
		const resource = resources.find(
			(r: Resource) => r.name === "patchResource",
		);
		expect(resource).toBeDefined();
	...
	it("should handle getFormProperties with unresolvable ref in properties", () => {
		...
		expect(resource.formProperties[0].schema).toEqual({
			$ref: "#/non/existent",
		});
	});
	...
	it("should correctly identify item vs collection actions based on parameters", () => {
		...
		const colAction = resource?.operations.find(
			(op) => op.action === "customCol",
		);
		const itemAction = resource?.operations.find(
			(op) => op.action === "customItem",
		);
	...
	it("should exclude non-primitive properties from listProperties", () => {
		...
		expect(resource).toBeDefined();
		expect(
			resource?.listProperties.some((p: FormProperty) => p.name === "id"),
		).toBe(true);
		expect(
			resource?.listProperties.some((p: FormProperty) => p.name === "data"),
		).toBe(false);
	});
});
