...
const mockDataGenSpec = {
	...
	components: {
		schemas: {
			...
			DeepNest1: {
				type: "object",
				properties: { nest2: { $ref: "#/components/schemas/DeepNest2" } },
			},
			DeepNest2: {
				type: "object",
				properties: { nest3: { $ref: "#/components/schemas/DeepNest3" } },
			},
			DeepNest3: {
				type: "object",
				properties: { nest4: { $ref: "#/components/schemas/DeepNest4" } },
			},
			DeepNest4: {
				type: "object",
				properties: { nest5: { $ref: "#/components/schemas/DeepNest5" } },
			},
			DeepNest5: {
				type: "object",
				properties: { nest6: { $ref: "#/components/schemas/DeepNest6" } },
			},
			DeepNest6: {
				type: "object",
				properties: { nest7: { $ref: "#/components/schemas/DeepNest7" } },
			},
			DeepNest7: {
				type: "object",
				properties: { nest8: { $ref: "#/components/schemas/DeepNest8" } },
			},
			DeepNest8: {
				type: "object",
				properties: { nest9: { $ref: "#/components/schemas/DeepNest9" } },
			},
			DeepNest9: {
				type: "object",
				properties: { nest10: { $ref: "#/components/schemas/DeepNest10" } },
			},
			DeepNest10: {
				type: "object",
				properties: { nest11: { $ref: "#/components/schemas/DeepNest11" } },
			},
...
describe("Generated Code: MockDataGenerator (Coverage)", () => {
	const createMockGenerator = (spec: object): MockDataGenerator => {
		...
		const documentUri = pathToFileURL(
			path.resolve(process.cwd(), "spec.json"),
		).href;
	...
	it("should handle externalValue by resolving local files", () => {
		(
			fs.existsSync as string | number | boolean | object | undefined | null
		).mockReturnValue(true);

		(
			fs.readFileSync as string | number | boolean | object | undefined | null
	...
	it("should prioritize dataValue, value, and serializedValue in that order", () => {
		expect(JSON.parse(generator.generate("DataValueSchema"))).toEqual({
			id: 1,
		});
		expect(JSON.parse(generator.generate("ValueSchema"))).toBe("raw-value");
		expect(JSON.parse(generator.generate("SerializedValueSchema"))).toBe(
			"serialized-value",
		);
	});
	...
	it("should fall back when externalValue file is missing or invalid", () => {
		...
		const invalid = JSON.parse(
			invalidBaseGenerator.generate("ExternalRelativeHttp"),
		);
		...
		(
			parser as string | number | boolean | object | undefined | null
		).documentUri = "";
		const customGenerator = new MockDataGenerator(parser);
