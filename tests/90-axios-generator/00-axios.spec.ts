...
describe("Axios Implementation", () => {
	describe("Config Validation", () => {
		...
		it("should execute AxiosClientGenerator successfully when admin is false", async () => {
			...
			const serviceFile = sourceFiles.find((f) =>
				f.getFilePath().endsWith("test.service.ts"),
			);
			...
			expect(getTestMethod?.getReturnTypeNode()?.getText()).toBe(
				"Promise<string>",
			);
			const statements = getTestMethod?.getStatements().map((s) => s.getText());
			const methodBody = statements.join(String.fromCharCode(10));
			expect(methodBody).toContain("axiosConfig");
		});
	});

	describe("AxiosClientGenerator and Index generation", () => {
		it("should export services properly from index", async () => {
			...
			const project = new Project();
			generateFromConfigSync(config, project, { spec });
			const mainIndex = project.getSourceFile(
				path.join(os.tmpdir(), "test-output/index.ts"),
			);
			expect(mainIndex).toBeDefined();
			expect(mainIndex?.getText()).toContain("./services");

			const servicesIndex = project.getSourceFile(
				path.join(os.tmpdir(), "test-output/services/index.ts"),
			);
			expect(servicesIndex).toBeDefined();
			expect(servicesIndex?.getText()).toContain(
				'export { TestService } from "./test.service";',
			);
		});
	});

	describe("AxiosServiceMethodGenerator", () => {
		it("should handle URL params, query params, headers, and body correctly", async () => {
			...
			expect(postBody).toContain("axiosConfig");
		...
		it("should handle blob, arraybuffer, and text response types", async () => {
			...
			expect(
				serviceClass
					...
					.map((s) => s.getText())
					.join("\\n"),
			).toContain("responseType = 'blob'"); // Octet/PDF translates to Blob by default parser mapping unless overriden, but wait, the parser logic dictates arraybuffer/blob.
			expect(
				serviceClass
					?.getMethod("getText")
					?.getStatements()
		...
		it("should generate properly without services when generateServices is false", async () => {
			...
			expect(
				project.getSourceFile(
					path.join(os.tmpdir(), "test-output-noservices/services/index.ts"),
				),
			).toBeUndefined();
		});
		...
		it("should generate properly with empty services index when no operations exist", async () => {
			...
			expect(
				project.getSourceFile(
					path.join(os.tmpdir(), "test-output-emptyops/services/index.ts"),
				),
			).toBeUndefined();
		});
	});
