...
describe("Vue Admin Generation", () => {
	it("should create admin directory when missing", () => {
		...
		const dirSpy = vi
			.spyOn(project.getFileSystem(), "directoryExistsSync")
			.mockReturnValue(false);
		const mkdirSpy = vi
			.spyOn(project.getFileSystem(), "mkdirSync")
			.mockImplementation(() => {});
	...
	it("should generate a full admin UI from coverageSpec and handle existing directories", async () => {
		...
		const listComponent = project.getSourceFile(
			"/generated/admin/users/UsersList.vue",
		);
		...
		const formComponent = project.getSourceFile(
			"/generated/admin/users/UsersForm.vue",
		);
		...
		const validatorsFile = project.getSourceFile(
			"/generated/admin/shared/validators.ts",
		);
		expect(validatorsFile).toBeDefined();

		const testFile = project.getSourceFile(
			"/generated/admin/users/users-list.component.spec.ts",
		);
	...
	it("should correctly generate a dynamic polymorphic form", async () => {
		...
		const formComponent = project.getSourceFile(
			"/generated/admin/pets/PetsForm.vue",
		);
		expect(formComponent).toBeDefined();
		const formText = formComponent?.getText();
	...
	it("should skip generation and warn if no valid admin resources are found", async () => {
		...
		expect(warnSpy).toHaveBeenCalledWith(
			expect.stringContaining("No resources suitable for admin UI generation"),
		);
	...
	it("should respect config.options.tests being false", async () => {
		...
		expect(
			project.getSourceFile("/generated/admin/users/UsersList.vue"),
		).toBeDefined();
		expect(
	...
	it("should generate a form that gracefully handles unresolvable allOf refs", async () => {
		...
		const formComponent = project.getSourceFile(
			"/generated/admin/badPets/BadPetsForm.vue",
		);
		expect(formComponent).toBeDefined();
