...
describe("E2E: Admin UI Generation", () => {
	...
	describe("Full Admin Generation from `coverageSpec`", () => {
		...
		beforeAll(async () => {
			...
			listComponent = project
				.getSourceFileOrThrow(
					"/generated/admin/users/users-list/users-list.component.ts",
				)
				.getClass("UsersListComponent")!;
			formComponent = project
				.getSourceFileOrThrow(
					"/generated/admin/users/users-form/users-form.component.ts",
				)
				.getClass("UserFormComponent")!;
			routingFile = project.getSourceFileOrThrow(
				"/generated/admin/users/users.routes.ts",
			)!;
		});
		it("should generate a fully-featured list component", () => {
			expect(listComponent).toBeDefined();
			const ngAfterViewInitBody =
				listComponent.getMethod("ngAfterViewInit")?.getBodyText() ?? "";
		...
		it("should generate a fully-featured form component", () => {
			...
			const initFormBody = formComponent
				.getMethodOrThrow("initForm")
				.getBodyText()!;

			expect(initFormBody).toContain(
			...
			const submitBody = formComponent
				.getMethodOrThrow("onSubmit")
				.getBodyText()!;
			expect(submitBody).toContain(
				"this.usersService.createUser(finalPayload)",
			);
			expect(submitBody).toContain(
				"this.usersService.updateUser(this.id()!, finalPayload)",
			);
		});
		...
		it("should generate a complete routing file for the resource", () => {
			const routesText =
				routingFile
					.getVariableDeclaration("usersRoutes")
					?.getInitializer()
					?.getText() ?? "";
		...
		it("should generate a master routing file with redirects", () => {
			const masterRoutes = project
				.getSourceFileOrThrow("/generated/admin/admin.routes.ts")
				.getText();
	...
	describe("Polymorphism E2E", () => {
		it("should correctly generate a dynamic polymorphic form", async () => {
			...
			const formComponent = project
				.getSourceFileOrThrow(
					"/generated/admin/pets/pets-form/pets-form.component.ts",
				)
				?.getClass("PetFormComponent")!;
			const html = project
				.getFileSystem()
				.readFileSync(
					"/generated/admin/pets/pets-form/pets-form.component.html",
				);
	...
	describe("Polymorphism E2E with Bad Refs", () => {
		it("should generate a form that gracefully handles unresolvable allOf refs", async () => {
					...
					"/bad-pets": {
						post: {
							...
							requestBody: {
								content: {
									"application/json": {
										schema: { $ref: "#/components/schemas/BadPet" },
									},
								},
							},
			...
			const html = project
				.getFileSystem()
				.readFileSync(
					"/generated/admin/badPets/badPets-form/badPets-form.component.html",
				);
