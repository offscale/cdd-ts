...
describe("Analysis: FormModelBuilder", () => {
	const setup = (
		...
		const resource: Resource = {
			...
			formProperties: Object.entries(mainSchema.properties || {}).map(
				([name, schema]) => ({
					name,
	...
	it("should extract keyPattern from patternProperties for Map types", () => {
		...
		const mapControl = result.topLevelControls.find(
			(c) => c.name === "prefixedMap",
		);
		expect(mapControl).toBeDefined();
		expect(mapControl?.controlType).toBe("map");
	...
	it("should extract dependentSchemas logic into dependencyRules", () => {
		...
		const rules = result.dependencyRules.filter(
			(r) => r.triggerField === "creditCard",
		);
						...
						properties: {
							poly: {
								...
								oneOf: [
									...
									{
										type: "object",
										properties: { inlineProp: { type: "string" } },
									},
	...
	it("should return early when dependentSchemas is missing", () => {
		...
		(
			builder as string | number | boolean | object | undefined | null
		).analyzeDependentSchemas({ type: "object" });

		expect(
			(builder as string | number | boolean | object | undefined | null).result
				.dependencyRules,
		).toEqual([]);
	});
	...
	it("should handle map schemas with empty patternProperties and unevaluatedProperties", () => {
		...
		const unevaluated = result.topLevelControls.find(
			(c) => c.name === "unevaluated",
		);
	...
	it("should fall back to Record<string, string | number | boolean | object | undefined | null> for invalid map value types", () => {
		...
		(
			builder as string | number | boolean | object | undefined | null
		).getFormControlTypeString = () =>
			"string | number | boolean | object | undefined | null";
		const result = builder.build(resource);
		const mapControl = result.topLevelControls.find((c) => c.name === "meta");

		expect(mapControl?.dataType).toBe(
			...
			components: {
				schemas: {
					Poly: {
						...
						discriminator: {
							propertyName: "type",
							defaultMapping: "#/components/schemas/Missing",
						},
					},
					Sub: {
						type: "object",
			...
			components: {
				schemas: {
					Poly: {
						...
						discriminator: {
							propertyName: "type",
							defaultMapping: "#/components/schemas/",
						},
					},
					Sub: {
						type: "object",
	...
	it("should support implicit discriminator mapping based on component name (OAS 3.2)", () => {
		...
		expect(result.isPolymorphic).toBe(true);
		expect(result.polymorphicProperties[0].discriminatorOptions).toContain(
			"Car",
		);
		expect(result.polymorphicProperties[0].discriminatorOptions).toContain(
			"Plane",
		...
		const carOption = result.polymorphicProperties[0].options.find(
			(o) => o.modelName === "Car",
		);
