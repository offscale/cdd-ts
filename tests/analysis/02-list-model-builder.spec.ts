...
describe("Analysis: ListModelBuilder", () => {
	...
	const createResource = (overrides: Partial<Resource>): Resource => {
		const defaultResource: Resource = {
			...
			isEditable: true,
			operations: [
				{ action: "list", methodName: "listItems" } as
					| string
					| number
					| boolean
					| object
	...
	it('should throw an error if the resource has no "list" operation', () => {
		...
		const resource = createResource({
			operations: [
				{ action: "create", methodName: "createItem" } as
					| string
					| number
					| boolean
					| object
	...
	it('should assign a "default" action kind for neutral custom action names', () => {
		...
		const getInfoAction = viewModel.customActions.find(
			(a) => a.name === "getInfo",
		);
		const processItemAction = viewModel.customActions.find(
			(a) => a.name === "processItem",
		);
	...
	it("should fall back to first property when id is missing", () => {
		...
		const resource = createResource({
			formProperties: [
				{ name: "uuid", schema: { type: "string" } as SwaggerDefinition },
			],
			listProperties: [
				{ name: "uuid", schema: { type: "string" } as SwaggerDefinition },
			],
	...
	it("should ensure a default id column when no usable properties exist", () => {
		...
		const resource = createResource({
			...
			formProperties: [
				{ name: "", schema: { type: "string" } as SwaggerDefinition },
			],
			listProperties: [],
		});
