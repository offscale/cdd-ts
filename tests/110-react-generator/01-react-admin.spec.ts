...
describe("ReactAdminGenerator", () => {
	...
	it("generates admin UI files successfully", async () => {
		...
		expect(
			project.getSourceFile("/out/admin/users/users-list.spec.tsx"),
		).toBeDefined();
		expect(
			project.getSourceFile("/out/admin/users/users-list.css"),
		).toBeDefined();
		expect(
			project.getSourceFile("/out/admin/shared/validators.ts"),
		).toBeDefined();
		expect(project.getSourceFile("/out/admin/shared/i18n.ts")).toBeDefined();
		expect(
			project.getSourceFile("/out/admin/users/users-form.tsx"),
		).toBeDefined();
	...
	it("generates only list components and skips forms when resources are read-only", async () => {
		...
		expect(
			project.getSourceFile("/out/admin/reports/reports-list.tsx"),
		).toBeDefined();
		// Should not generate form for read-only resource
		expect(
			project.getSourceFile("/out/admin/reports/reports-form.tsx"),
		).toBeUndefined();
	...
	it("should respect config.options.tests being false", () => {
		...
		expect(
			project.getSourceFile("/out/admin/users/users-list.tsx"),
		).toBeDefined();
		expect(
			project.getSourceFile("/out/admin/users/users-list.spec.tsx"),
		).toBeUndefined();
...
describe("RoutingGenerator", () => {
	it("generates routes and app shell", () => {
		...
		expect(routeFile.getText()).toContain(
