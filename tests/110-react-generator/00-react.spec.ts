...
describe("React Implementation", () => {
	...
	it("should generate React hooks for services", async () => {
		...
		const config = {
			options: { admin: false, tests: true },
		} as GeneratorConfig;
		...
		expect(hooksIndex?.getFullText()).toContain(
			'export { useUsersService } from "./users.hook.js";',
		);
		...
		const collisionHookText = project
			.getSourceFile("/output/hooks/collision.hook.ts")
			?.getFullText();
		expect(collisionHookText).toContain("useGetFooBar");
		expect(collisionHookText).toContain("useGetFooBar2");

		const invalidHookText = project
			.getSourceFile("/output/hooks/invalid.hook.ts")
			?.getFullText();
	...
	it("should respect config.options.tests being false in hook generation", async () => {
		...
		expect(
			project.getSourceFile("/out/hooks/users.hook.spec.ts"),
		).toBeUndefined();
	});
});
