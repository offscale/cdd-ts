...
describe("TypeOrmGenerator", () => {
	it("should generate an entity file correctly with explicit types and JSDoc", async () => {
		...
		const testFile = project.getSourceFileOrThrow(
			"/out/entities/user.entity.spec.ts",
		);
	...
	it("should handle schemas without properties safely", async () => {
		...
		const file = project.getSourceFileOrThrow(
			"/out/entities/noprops.entity.ts",
		);
		const text = file.getFullText();
	...
	it("should handle optional id and missing required array", async () => {
		...
		const file = project.getSourceFileOrThrow(
			"/out/entities/optionalid.entity.ts",
		);
		const text = file.getFullText();
	...
	it("should respect config.options.tests being false", () => {
		...
		expect(
			project.getSourceFile("/out/entities/user.entity.spec.ts"),
		).toBeUndefined();
	});
});
