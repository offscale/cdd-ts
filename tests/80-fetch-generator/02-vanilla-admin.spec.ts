...
describe("Vanilla Admin UI Generation (Fetch)", () => {
	it("should generate vanilla web components for admin UI", async () => {
		...
		expect(
			files.some((f) =>
				f.includes("admin/users/users-list/users-list.component.ts"),
			),
		).toBe(true);
		expect(
			files.some((f) =>
				f.includes("admin/users/users-form/users-form.component.ts"),
			),
		).toBe(true);

		const appShell = project.getSourceFileOrThrow(
			"/generated/admin/app-shell.ts",
		);
		expect(appShell.getText()).toContain(
			'import "./users/users-list/users-list.component.js"',
		);
		expect(appShell.getText()).toContain(
			'import "./users/users-form/users-form.component.js"',
		);
	});
});
