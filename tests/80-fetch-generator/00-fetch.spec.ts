...
describe("Fetch Implementation", () => {
	...
	describe("FetchClientGenerator and Index generation", () => {
	...
	describe("FetchServiceMethodGenerator", () => {
		it("should handle URL params, query params, headers, and body correctly", async () => {
			...
			expect(postBody).toContain("fetch(");
		...
		it("should handle blob, arraybuffer, and text response types", async () => {
			...
			expect(
				serviceClass
					...
					.map((s) => s.getText())
					.join("\\n"),
			).toContain("response.blob()"); // Octet/PDF translates to Blob by default parser mapping unless overriden, but wait, the parser logic dictates arraybuffer/blob.
			expect(
				serviceClass
					?.getMethod("getText")
					?.getStatements()
		...
		it("should generate properly without services when generateServices is false", async () => {
			const config: GeneratorConfig = {
				...
				options: { implementation: "fetch", generateServices: false },
			};
