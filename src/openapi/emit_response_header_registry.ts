...
export class ResponseHeaderRegistryGenerator {
			...
			Object.entries(op.responses).forEach(([statusCode, response]) => {
				if (response.headers) {
					...
					Object.entries(response.headers).forEach(
						([headerName, headerOrRef]) => {
							const headerDef = this.parser.resolve(
								headerOrRef,
							) as HeaderObject;
							...
							headerObjects[headerName] = headerOrRef as
								| HeaderObject
								| { $ref: string };
		...
		if (headerCount > 0 || headerObjectCount > 0) {
			...
			if (headerObjectCount > 0) {
				sourceFile.addVariableStatement({
					...
					docs: [
						"Full Response Header Objects for reverse OpenAPI generation.",
					],
				});
			}
				...
				declarations: [
					{
						...
						initializer:
							Object.keys(headerConfigMap).length > 0
	...
	private getHeaderTypeInfo(header: HeaderObject): {
		typeHint: string;
	...
	private getXmlConfig(
		...
		if (Array.isArray(resolved.prefixItems)) {
			config.prefixItems = resolved.prefixItems.map((item: OpenApiValue) =>
