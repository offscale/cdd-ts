...
export function groupPathsByController(parser: {
	operations: PathInfo[];
...
export function extractPaths(
	...
	const resolveHeaders = (
		headers:
			| Record<
					string,
					| HeaderObject
					| { $ref?: string; $dynamicRef?: string; description?: string }
			  >
			| undefined,
	...
	const resolveContentMap = (
		content:
			| Record<
					string,
					MediaTypeObject | { $ref?: string; $dynamicRef?: string }
			  >
			| undefined,
		...
		return Object.keys(resolvedContent).length > 0
			? resolvedContent
			: undefined;
	};

	const securitySchemeNames = new Set(
		components?.securitySchemes ? Object.keys(components.securitySchemes) : [],
	);
	...
	for (const [path, rawPathItem] of Object.entries(swaggerPaths)) {
		...
		if (pathItem.$ref && (resolveObj || resolveRef)) {
			const resolvedByObj =
				resolveObj && typeof pathItem === "object"
					? (resolveObj(pathItem) as PathItem | undefined)
					: undefined;
			const resolved =
				resolvedByObj ??
				(resolveRef
					? (resolveRef(pathItem.$ref) as PathItem | undefined)
					: undefined);
		...
		const rawPathParameters: UnifiedParameter[] = Array.isArray(
			pathItem.parameters,
		)
		...
		const pathParameters: UnifiedParameter[] = rawPathParameters
			...
			.filter(
				(param): param is UnifiedParameter =>
		...
		for (const { method, operation } of operationsToProcess) {
			...
			(Array.isArray(operation.parameters) ? operation.parameters : []).forEach(
				(p) => {
					const resolvedParam = resolveMaybeRef<UnifiedParameter>(
						p as UnifiedParameter,
					);

					if (
						resolvedParam &&
						"name" in resolvedParam &&
						"in" in resolvedParam
					) {
						paramsMap.set(
							`${resolvedParam.name}:${resolvedParam.in}`,
							resolvedParam,
						);
					}
				},
			...
			const bodyParam = allParams.find((p) => p && p.in === "body") as
				| BodyParameter
				| undefined;
			...
			const parameters = nonBodyParams
				...
				.map((p): Parameter => {
					...
					const resolvedContent = resolveContentMap(
						p.content as Record<string, MediaTypeObject>,
					);
					...
					if (resolvedContent) {
						...
						if (
							contentType &&
							resolvedContent[contentType].schema !== undefined
						) {
					...
					if (finalSchema === undefined) {
						...
						if (p.type !== undefined)
							baseSchema.type = p.type as Exclude<
								SwaggerDefinition["type"],
								undefined
							>;
					...
					if (p.examples !== undefined)
						param.examples = p.examples as Record<string, ExampleObject>;
			...
			if (operation.requestBody !== undefined) {
				...
				consumes.forEach((mediaType) => {
					content[mediaType] = {
						schema: bodyParam.schema as OpenApiValue as SwaggerDefinition,
					};
				});
				...
				if (bodyParam.required !== undefined)
					requestBody.required = bodyParam.required;
			}

			if (requestBody?.content) {
				const resolvedContent = resolveContentMap(
					requestBody.content as Record<string, MediaTypeObject>,
				);
			...
			if (operation.responses) {
				for (const [code, resp] of Object.entries(operation.responses)) {
					const resolvedResp =
						resolveMaybeRef<SwaggerResponse | Response>(
							resp as SwaggerResponse | Response,
						) ?? (resp as SwaggerResponse);

					const swagger2Response = resolvedResp as Response;

					const headers = resolveHeaders(
						(resolvedResp as Record<string, OpenApiValue>).headers as Record<
							string,
							HeaderObject
						>,
					);
					...
					if (swagger2Response && swagger2Response.schema !== undefined) {
						...
						produces.forEach((mediaType) => {
							content[mediaType] = {
								schema:
									swagger2Response.schema as OpenApiValue as SwaggerDefinition,
							};
						});
						...
						const resolvedContent = resolveContentMap(
							(resolvedResp as SwaggerResponse).content as Record<
								string,
								MediaTypeObject
							>,
						);
			...
			const effectiveServers =
				operation.servers !== undefined ? operation.servers : pathServers;
			...
			if (operation.callbacks)
				pathInfo.callbacks = operation.callbacks as Record<string, PathItem>;
			...
			if (operation.externalDocs)
				pathInfo.externalDocs = operation.externalDocs;
