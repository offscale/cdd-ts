...
export class AxiosClientGenerator extends AbstractClientGenerator {
    ...
    public async generate(
        ...
        if (config.options.generateServices ?? true) {
            ...
            // Generate Services
            /* v8 ignore next */
            new AxiosServiceGenerator(parser, project, config).generate(servicesDir, controllerGroups);
            /* v8 ignore next */
        ...
        /* v8 ignore next */
        new AxiosMainIndexGenerator(project, config, parser).generateMainIndex(outputRoot);
    }
}
