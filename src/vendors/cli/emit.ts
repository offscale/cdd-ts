...
export class CliGenerator {
	...
	public generate(
		...
		const cliFile = project.createSourceFile(
			path.join(outputDir, "cli.ts"),
			"",
			{ overwrite: true },
		);
		...
		const description = (
			info.description ||
			info.summary ||
			"CLI for API generated by cdd-ts"
		).replace(/'/g, "\\'");
			...
			for (const [name, scheme] of Object.entries(
				...
				if (s.type === "http" || s.type === "apiKey") {
					...
					if (s.oauth2MetadataUrl)
						statements += `// OAuth2 Metadata: ${s.oauth2MetadataUrl}\n`;
				...
				if (s.deprecated)
					statements += `// Deprecated security scheme: ${name}\n`;
			}
		}

		const grouped = parser.operations.reduce(
			(acc: Record<string, PathInfo[]>, op) => {
...
`;
			...
			for (const op of ops) {
				...
				const opDesc = (op.summary || op.description || "").replace(
					/'/g,
					"\\'",
				);
				...
				if (op.parameters) {
					for (const param of op.parameters) {
						...
						const requiredFlag = (param as Parameter).required
							? "<value>"
							: "[value]";
						const pDesc = ((param as Parameter).description || "").replace(
							/'/g,
							"\\'",
						);
