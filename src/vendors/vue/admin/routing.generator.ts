...
export class RoutingGenerator {
	...
	public generate(resource: Resource, adminDir: string): void {
		...
		sourceFile.addImportDeclarations([
			{ moduleSpecifier: "vue-router", namedImports: ["RouteRecordRaw"] },
		]);
		...
		if (resource.operations.some((op) => op.action === "list")) {
			...
			routes.push(
				`{ path: '/${resource.name}', component: List, name: '${resource.name}List' }`,
			);
		}
		...
		if (resource.isEditable) {
			...
			routes.push(
				`{ path: '/${resource.name}/new', component: Form, name: '${resource.name}Create' }`,
			);
			routes.push(
	...
	public generateMaster(resources: Resource[], adminDir: string): void {
		...
		const links = resources
			...
			.map(
				(r) =>
					`        <router-link to="/${r.name}" class="nav-link">${r.name}</router-link>`,
			)
			.join("\n");
...
`.trim();
		...
		this.project.createSourceFile(appFilePath, appTemplate, {
			overwrite: true,
		});

		const routerFilePath = path.posix.join(adminDir, "router.ts");
		const routerFile = this.project.createSourceFile(routerFilePath, "", {
			overwrite: true,
		});
		...
		routerFile.addImportDeclarations([
			{
				...
				namedImports: ["createRouter", "createWebHistory", "RouteRecordRaw"],
			},
		]);
		...
		resources.forEach((r) => {
			if (r.operations.some((op) => op.action === "list") || r.isEditable) {
				routeImports.push(
					`import { routes as ${r.name}Routes } from './${r.name}/routes.js';`,
				);
