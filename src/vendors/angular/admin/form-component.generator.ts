...
export class FormComponentGenerator {
	...
	private generateFormComponentTs(
		...
		const sourceFile = this.project.createSourceFile(formFilePath, undefined, {
			overwrite: true,
		});
		// The builder creates the main interface with this name
		const formInterfaceName =
			analysis.interfaces.find((i) => i.isTopLevel)?.name ||
			`${pascalCase(resource.modelName)}Form`;

		// Collect all polymorphic models for import

		const oneOfImports = analysis.polymorphicProperties
			.flatMap((p) => p.options.map((o) => o.modelName))
			.join(", ");
		...
		analysis.interfaces.forEach((iface) => {
			sourceFile.addInterface({
				...
				properties: iface.properties.map((prop) => {
					...
					const control = findControlInModel(
					...
					switch (control.controlType) {
						...
						case "array":
							if (control.nestedControls) {
								...
								const itemType = control.dataType
									.replace(/[()]/g, "")
									.replace("[]", "");
		...
		if (analysis.isPolymorphic)
			this.addPolymorphismLogic(componentClass, resource, analysis);

		if (analysis.hasFileUploads) this.addFileHandling(componentClass);

		if (analysis.hasFormArrays)
			this.addFormArrayHelpers(componentClass, analysis.topLevelControls);

		if (analysis.hasMaps)
			this.addMapHelpers(componentClass, analysis.topLevelControls);
	...
	private addProperties(
				...
				classDeclaration.addProperties([
					{
						...
						docs: [
							`The available options for the ${poly.propertyName} discriminator.`,
						],
					},
				]);
			});
		...
		const findEnums = (properties: FormControlModel[]) => {
			for (const prop of properties) {
				...
				const itemsSchema = (schema.type === "array" ? schema.items : schema) as
					| SwaggerDefinition
					| undefined;
	...
	private addDynamicFormEffectsMethod(
		classDeclaration: ClassDeclaration,
		analysis: FormAnalysisResult,
	): void {
		classDeclaration.addMethod({
			...
			statements: (writer) => {
				...
				if (analysis.dependencyRules.length > 0) {
					writer.writeLine(
						"// Dependent Schemas: If trigger field is present, target is required.",
					);
					...
					groups.forEach((targets, trigger) => {
						...
						targets.forEach((target) => {
							writer.writeLine(
								`    this.form.get('${target}')?.addValidators(Validators.required);`,
							);
							writer.writeLine(
	...
	private addInitForm(
		...
		const formControls = controls
			...
			.map(
				(c) =>
					`'${c.name}': ${FormInitializerRenderer.renderControlInitializer(c)}`,
			)
			.join(",\n      ");
		...
		if (readOnlyPaths.length > 0) {
			const disableStats = readOnlyPaths
				...
				.map(
					(path) => `this.form.get('${path}')?.disable({ emitEvent: false });`,
				)
				.join("\n    ");
	...
	private findReadOnlyControls(
		...
		for (const control of controls) {
			...
			if (control.nestedControls && control.controlType === "group") {
				...
				paths = paths.concat(
					this.findReadOnlyControls(control.nestedControls, controlPath),
				);
			}
		}
	...
	private addFormArrayHelpers(
		...
		formArrayProps.forEach((prop) => {
			...
			const initializerString =
				FormInitializerRenderer.renderFormArrayItemInitializer(
					prop.nestedControls!,
				);
	...
	private addOnSubmit(
		...
		const payloadExpr = needsPayloadTransform
			? "this.getPayload()"
  ...
  error: (err) => {
	...
	private addPatchForm(
		classDeclaration: ClassDeclaration,
		resource: Resource,
		analysis: FormAnalysisResult,
	): void {
		const arrayProps = analysis.topLevelControls.filter(
			(c) => c.controlType === "array" && c.nestedControls,
		);

		const mapProps = analysis.topLevelControls.filter(
			(c) => c.controlType === "map",
		);
	...
	private addGetPayload(
		...
		const maps = analysis.topLevelControls.filter(
			(c) => c.controlType === "map",
		);
	...
	private generateFormComponentHtml(
		resource: Resource,
		analysis: FormAnalysisResult,
		outDir: string,
	): void {
