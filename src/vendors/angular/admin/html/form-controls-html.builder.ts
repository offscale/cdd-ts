...
export function buildFormControl(
	control: FormControlModel,
): HtmlElementBuilder | null {
	if (!control?.schema) return null;
	...
	switch (control.schema.type) {
		case "string":
			...
			if (control.schema.format === "binary")
				return createFile(control, labelText);

			if (control.schema.format === "textarea")
				return createTextarea(control, labelText);
			...
			if (control.schema.enum)
				return control.schema.enum.length > 4
					? createSelect(
							control,
							labelText,
							`${camelCase(control.name)}Options`,
							false,
						)
					: createRadio(
							control,
		...
		case "number":
			if (
				...
				return createSlider(
					control,
					labelText,
					control.schema.minimum,
					control.schema.maximum,
				);
			}
		...
		case "object":
			if (control.controlType === "map")
				return createMapEditor(control, labelText);
...
function createInput(
	...
	field.appendChild(
		_.create("mat-label").setAttribute("i18n", "").setTextContent(label),
	);
...
function createDatepicker(
	...
	field.appendChild(
		_.create("mat-datepicker-toggle")
			.setAttribute("matSuffix", "")
			.setAttribute("[for]", pickerId),
	);

	field.appendChild(
		_.create("mat-datepicker").setAttribute(`#${pickerId}`, ""),
	);
...
function createSelect(
	...
	const select = _.create("mat-select").setAttribute(
		"formControlName",
		control.name,
	);
...
function createFile(
	...
	container.appendChild(
		_.create("span")
			...
			.setTextContent(
				`{{ form.get('${control.name}')?.value?.name || 'No file selected' }}`,
			),
	);
...
function createFormGroup(control: FormControlModel): HtmlElementBuilder {
	const container = _.create("div")
		.addClass("admin-form-group")
		.setAttribute("formGroupName", control.name);

	container.appendChild(
		_.create("h3")
			.setAttribute("i18n", "")
			.setTextContent(pascalCase(control.name)),
	);
...
function createFormArray(
	...
	container.appendChild(
		_.create("h3").setAttribute("i18n", "").setTextContent(label),
	);

	const arrayContainer = _.create("div").setAttribute(
		"formArrayName",
		control.name,
	);

	const itemWrapper = _.create("div");

	const itemContainer = _.create("div").setAttribute("[formGroupName]", "i");
	...
	const innerItemHtml = itemContainer.render(1);
	itemWrapper.setInnerHtml(
		`@for (item of ${camelCase(control.name)}Array.controls; track $index; let i = $index) {\n${innerItemHtml}\n}`,
	);
...
function createMapEditor(
	...
	const loopWrapper = _.create("div");

	const loopContainer = _.create("div")
		.setAttribute("[formGroupName]", "i")
		.addClass("map-pair-row");

	const keyField = _.create("mat-form-field").addClass("map-key-field");

	keyField.appendChild(
		_.create("mat-label").setAttribute("i18n", "").setTextContent("Key"),
	);

	keyField.appendChild(
		_.create("input")
			.setAttribute("matInput", "")
			.setAttribute("formControlName", "key"),
	);
	...
	const innerLoopHtml = loopContainer.render(1);
	loopWrapper.setInnerHtml(
		`@for (pair of ${mapGetter}.controls; track $index; let i = $index) {\n${innerLoopHtml}\n}`,
	);

	arrayContainer.appendChild(loopWrapper);
