...
export function generateListComponentHtml(
	...
	const customCollectionActions = resource.operations.filter(
		(op) => op.isCustomCollectionAction,
	);

	const customItemActions = resource.operations.filter(
		(op) => op.isCustomItemAction,
	);
	...
	const toolbar = _.create("mat-toolbar")
		.addClass("admin-list-toolbar")
		.setAttribute("role", "toolbar");

	toolbar.appendChild(
		_.create("span")
			.setAttribute("i18n", "")
			.setTextContent(pascalCase(resourceName)),
	);
	...
	const tableContainer = _.create("div").addClass(
		"mat-elevation-z8 table-container",
	);
	...
	const columnNames = [
		...new Set([idProperty, ...listableProps.map((p) => p.name)]),
	].filter(Boolean);
	...
	table.appendChild(
		_.create("tr")
			.setAttribute("mat-header-row", "")
			.setAttribute("*matHeaderRowDef", "displayedColumns"),
	);

	table.appendChild(
		_.create("tr")
			.setAttribute("mat-row", "")
			.setAttribute("*matRowDef", "let row; columns: displayedColumns;"),
	);
