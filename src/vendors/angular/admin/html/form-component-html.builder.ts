...
export function generateFormComponentHtml(
	resource: Resource,
	analysis: FormAnalysisResult,
): string {
	const root = _.create("div").addClass("admin-form-container");

	const title = _.create("h1")
		.setAttribute("i18n", "")
		.setTextContent("{{formTitle()}}");
	...
	const discriminatorProps = analysis.polymorphicProperties.map(
		(p) => p.propertyName,
	);
	...
	if (analysis.isPolymorphic) {
		for (const poly of analysis.polymorphicProperties) {
			...
			const discriminatorControl = analysis.topLevelControls.find(
				(c) => c.name === poly.propertyName,
			);
			...
			for (const option of poly.options || []) {
				...
				const formGroupContainer = _.create("div").setAttribute(
					"formGroupName",
					typeName,
				);
				...
				ifContainer.setInnerHtml(
					`@if (${checkMethod}('${typeName}')) {\n${innerHtml}\n  }`,
				);
	...
	const actionsContainer = _.create("div")
		.addClass("admin-form-actions")
