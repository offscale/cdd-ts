...
export class FormModelBuilder {
	...
	public build(resource: Resource): FormAnalysisResult {
		...
		const modelDef = definitions.find(
			(d) => d.name === resource.modelName,
		)?.definition;
	...
	private analyzeDependentSchemas(modelSchema: SwaggerDefinition) {
		...
		Object.entries(modelSchema.dependentSchemas).forEach(
			([triggerProp, schemaOrRef]) => {
				const dependentSchema = this.parser.resolve(
					schemaOrRef as SwaggerDefinition,
				);
	...
	private analyzeDependentRequired(modelSchema: SwaggerDefinition) {
		...
		Object.entries(modelSchema.dependentRequired).forEach(
			([triggerProp, requiredList]) => {
		...
		for (const prop of properties) {
			...
			if (schema.type === "object" && schema.properties) {
				...
				if (schema.propertyNames && typeof schema.propertyNames === "object") {
					const resolvedPropertyNames = this.parser.resolve(
						schema.propertyNames as SwaggerDefinition,
					);
				...
				if (
					!rawValueSchema ||
					(typeof rawValueSchema === "object" &&
						Object.keys(rawValueSchema).length === 0)
				) {
					rawValueSchema =
				...
				const valueControls = this.analyzeControls(
					[
						{
							name: valuePropName,
							schema: rawValueSchema as SwaggerDefinition,
						},
					],
				...
				controlModel = {
					...
					schema,
					...(valueControl.nestedFormInterface && {
						nestedFormInterface: valueControl.nestedFormInterface,
					}),
				...
				const itemSchema = (schema.items ?? schema.unevaluatedItems ?? {}) as
					| SwaggerDefinition
					| boolean;
	...
	private analyzePolymorphism(
		prop: FormProperty,
	): PolymorphicPropertyConfig | null {
		if (
			!prop.schema ||
			typeof prop.schema !== "object" ||
			!prop.schema.discriminator
		)
			return null;
		...
		for (const subSchemaRef of prop.schema.oneOf || []) {
			const refString =
				typeof subSchemaRef === "object"
					? subSchemaRef.$ref || subSchemaRef.$dynamicRef
					: undefined;
			...
			const allProperties: Record<string, SwaggerDefinition> = {
				...(((subSchema as SwaggerDefinition).properties || {}) as Record<
					string,
					SwaggerDefinition
				>),
			};
			...
			if (!typeName) {
				const mappedKey = Object.keys(explicitMapping).find(
					(key) => explicitMapping[key] === refString,
				);
			...
			for (const subProp of subProperties) {
				const controls = this.analyzeControls(
					[subProp],
					`Temp${refName}`,
					false,
				);
		...
		if (prop.schema.discriminator?.defaultMapping) {
			const defaultName = pascalCase(
				prop.schema.discriminator.defaultMapping.split("/").pop() || "",
			);

			if (
				defaultName &&
				config.options.some((p) => p.modelName === defaultName)
			) {
	...
	private getFormControlTypeString(
