...
export class ListComponentGenerator {
	...
	private emitListComponentTs(
		...
		const sourceFile = this.project.createSourceFile(
			`${outDir}/${model.resourceName}-list.component.ts`,
			"",
			{
				overwrite: true,
			},
		...
		sourceFile.addStatements([
			...
			...commonStandaloneImports.map(
				(a) => `import { ${a[0]} } from "${a[1]}";`,
			),
		]);
	...
	private addProperties(
		...
		componentClass.addProperties([
			...
			},
			{
				name: "i18n",
				isReadonly: true,
				initializer: "inject(TranslationService)",
			},
			{
			...
			{
				name: "displayedColumns: string[]",
				initializer: JSON.stringify(model.displayedColumns),
			},
			{ name: "idProperty: string", initializer: `'${model.idProperty}'` },
		]);
	}

	private addLifecycleAndUtilityMethods(
		componentClass: ClassDeclaration,
		model: ListViewModel,
	): void {
	...
	private emitListComponentHtml(
		...
		model.customActions.forEach((a) =>
			iconMap.set(a.name, this.mapKindToIcon(a.name, a.kind)),
		);

		// Delegate to HTML builder.

		const htmlContent = generateListComponentHtml(
			resource,
			model.idProperty,
			iconMap,
		);
		...
		this.project
			.getFileSystem()
			.writeFileSync(
				`${outDir}/${model.resourceName}-list.component.html`,
				htmlContent,
			);
	}
	...
	private emitListComponentScss(model: ListViewModel, outDir: string): void {
		...
		this.project
			.getFileSystem()
			.writeFileSync(
				`${outDir}/${model.resourceName}-list.component.scss`,
				scssContent,
			);
	}
}
