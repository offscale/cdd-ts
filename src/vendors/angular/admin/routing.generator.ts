...
export class RoutingGenerator {
    ...
    loadChildren: () => import('./${resource.name}/${resource.name}.routes').then(m => m.${camelCase(resource.name)}Routes) 
		...
		if (resources.length > 0) {
			routeObjects.unshift(
				`{ path: '', pathMatch: 'full', redirectTo: '${resources[0].name}' }`,
			);
		}
	...
	public generate(resource: Resource, outDir: string): void {
		...
		const hasEdit = resource.operations.some(
			(op) => op.action === "getById" || op.action === "update",
		);
		...
		if (hasList) {
			...
			routes.push(
				`{ path: '', loadComponent: () => import('${componentPath}').then(m => m.${componentName}) }`,
			);
		}
