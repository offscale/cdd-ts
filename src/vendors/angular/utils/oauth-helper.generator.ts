...
export class OAuthHelperGenerator {
	...
	public generate(outputDir: string): void {
		...
		const oauthSchemes = securitySchemes.filter(
		...
		for (const scheme of oauthSchemes) {
			if (scheme.type === "openIdConnect") {
				...
				if (passwordFlow) {
					...
					if (!config.tokenUrl && passwordFlow.tokenUrl)
						config.tokenUrl = passwordFlow.tokenUrl;
				}
				...
				if (ccFlow) {
					...
					if (!config.tokenUrl && ccFlow.tokenUrl)
						config.tokenUrl = ccFlow.tokenUrl;
				}
				...
				if (acFlow) {
					...
					if (!config.tokenUrl && acFlow.tokenUrl)
						config.tokenUrl = acFlow.tokenUrl;
				}

				const devFlow = scheme.flows.deviceAuthorization as Record<
					string,
					string
				>;
				...
				if (devFlow) {
					...
					if (!config.tokenUrl && devFlow.tokenUrl)
						config.tokenUrl = devFlow.tokenUrl;
				}
			}
		}
	...
	private generateService(authDir: string, config: OAuthFlowConfig): void {
		...
		const imports = [
			...
			{ moduleSpecifier: "@angular/router", namedImports: ["Router"] },
			{
				...
				namedImports: ["OAuthService", "AuthConfig"],
			},
		];
		...
		if (
			...
			config.hasDeviceAuthorization
		) {
			imports.push({
				moduleSpecifier: "@angular/common/http",
				namedImports: ["HttpClient", "HttpHeaders"],
			});
		}
		...
		const serviceClass = sourceFile.addClass({
			...
			isExported: true,
			decorators: [
				{ name: "Injectable", arguments: [`{ providedIn: 'root' }`] },
			],
	...
	private addCommonMethods(serviceClass: ClassDeclaration): void {
		serviceClass.addMethods([
			...
			},
			{
				...
				isAsync: true,
				parameters: [
					{ name: "config", type: "AuthConfig", hasQuestionToken: true },
				],
	...
	private addAuthCodeMethods(serviceClass: ClassDeclaration): void {
		serviceClass.addMethod({
			...
			parameters: [
				{ name: "redirectUrl", type: "string", hasQuestionToken: true },
			],
	...
	private addPasswordMethods(serviceClass: ClassDeclaration): void {
		serviceClass.addMethod({
			name: "loginPassword",
			docs: [
				"Exchanges username/password for a token (Resource Owner Password Flow).",
			],
			parameters: [
			...
			returnType:
				"Promise<Record<string, string | number | boolean | object | undefined | null>>",
			statements: `
        ...
        return new Promise((resolve, reject) => { 
            this.http.post(this.tokenUrl, body.toString(), { 
                ...
                next: (res: { access_token?: string } & Record<string, string | number | boolean | object | undefined | null>) => { 
	...
	private addDeviceAuthorizationMethods(serviceClass: ClassDeclaration): void {
        ...
        return new Promise((resolve, reject) => { 
            this.http.post(this.deviceAuthorizationUrl, body.toString(), { 
                ...
                next: (res: Record<string, string | number | boolean | object | undefined | null>) => resolve(res), 
                error: reject
            }); 
        });`,
		});
	...
	private generateRedirectComponent(authDir: string): void {
		...
		const tsFile = this.project.createSourceFile(tsPath, "", {
			overwrite: true,
		});
		...
		tsFile.addImportDeclarations([
			...
			{
				moduleSpecifier: "../oauth.service",
				namedImports: ["OAuthHelperService"],
			},
		]);
		...
		this.project
			.getFileSystem()
			.writeFileSync(htmlPath, `<p>Redirecting...</p>`);
	}
}
