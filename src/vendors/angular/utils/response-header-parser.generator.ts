...
export class ResponseHeaderParserGenerator {
	...
	public generate(outputDir: string): void {
		...
		const serviceClass = sourceFile.addClass({
			...
			docs: [
				"Service to parse and coerce response headers into typed objects based on API metadata.",
			],
		});
        ...
        Object.entries(headerConfig).forEach(([headerName, typeHint]) => {
            ...
            if (typeHint === 'array' || typeHint === 'set-cookie') {
                ...
                if (val !== null) {
                    ...
                    const xmlConfig = (API_HEADER_XML_CONFIGS as Record<string, string | number | boolean | object | undefined | null>)[xmlConfigKey];
