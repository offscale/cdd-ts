...
export class ProviderGenerator {
	...
	public generate(outputDir: string): void {
		...
		const hasSecurity =
			this.hasApiKey || this.hasCookieAuth || this.hasBearer || this.hasMtls;
	...
	private addConfigInterface(sourceFile: SourceFile): void {
			...
			properties: [
				{
					...
					docs: [
						"The base path of the API endpoint. If provided, it overrides the default server URL.",
					],
				},
				{
					...
					hasQuestionToken: true,
					docs: [
						"Values for server variables (e.g. { port: '8080' }) to resolve the default server URL.",
					],
				},
				{
					...
					hasQuestionToken: true,
					docs: [
						"If true, automatically transforms ISO date strings. Default: true",
					],
				},
				{
					name: "interceptors",
		...
		if (this.hasMtls) {
			configInterface.addProperty({
				...
				hasQuestionToken: true,
				docs: [
					"Configuration for the HTTPS Agent (e.g. PFX, Cert, Key) for Mutual TLS.",
				],
			});
		}
	}
	...
	private addMainProviderFunction(
		...
		sourceFile.addFunction({
			...
			isExported: true,
			parameters: [
				{ name: "config", type: `${this.capitalizedClientName}Config` },
			],
			...
			statements: (writer) => {
				...
				writer.writeLine(
					`fns.push(${camelCase(this.clientName)}BaseInterceptor);`,
				);
				if (hasSecurity) {
				...
				if (this.config.options.dateType === "Date") {
					writer
						.write("if (config.enableDateTransform !== false)")
						.block(() => {
							writer.writeLine("fns.push(dateInterceptor);");
						});
				}
				writer.writeLine(
					"providers.push(provideHttpClient(withInterceptors(fns)));",
				);
