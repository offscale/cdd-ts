...
export class ServiceMethodGenerator {
	...
	public addServiceMethod(
		...
		const negotiationVariants = this.getDistinctNegotiationVariants(
		...
		const bodyStatements = this.emitMethodBody(
			model,
			operation,
			isSSE,
		...
		if (
			(hasContentNegotiation || hasMultipleSuccessTypes) &&
			overloads.length === 0
		) {
	...
	private buildParamTags(
		...
		const paramNames = new Set(
			parameters
				...
				.filter(
					(name): name is string =>
						typeof name === "string" && name.length > 0 && name !== "options",
				),
		);

		const opParamNames = new Set(
			(operation.parameters ?? [])
				.map((param) => camelCase(param.name))
	...
	private buildExampleTags(
		...
		if (bodyParam?.name && operation.requestBody) {
			const requestExamples = this.extractRequestBodyExamples(
		...
		if (operation.responses) {
			Object.entries(operation.responses).forEach(([code, response]) => {
				const resolved =
					this.parser.resolve<SwaggerResponse>(response as ReferenceLike) ??
	...
	private extractExampleValue(
		...
		if (preferSerialized) {
			if (Object.hasOwn(exampleObj, "serializedValue")) {
				return {
					found: true,
					value: (
			...
			if (Object.hasOwn(exampleObj, "externalValue")) {
				return {
					found: true,
					value: (
						exampleObj as Record<
			...
			if (Object.hasOwn(exampleObj, "dataValue")) {
				return {
					found: true,
					value: (
						exampleObj as Record<
			...
			if (Object.hasOwn(exampleObj, "value")) {
				return {
					found: true,
					value: (
						exampleObj as Record<
	...
	private extractParameterExample(param: Parameter): OpenApiValue | undefined {
		...
		if (param.examples && typeof param.examples === "object") {
			...
			if (firstExample !== undefined) {
				const resolved =
					this.parser.resolve(firstExample as ReferenceLike) ?? firstExample;
		...
		if (
			...
			const schema = param.schema as Record<
				string,
				string | number | boolean | object | undefined | null
			>;
		...
		if (param.content) {
			...
			const example = mediaObj
				? this.extractMediaTypeExample(mediaObj, mediaType)
				: undefined;
	...
	private extractMediaTypeExample(
		...
		const resolved =
			this.parser.resolve<MediaTypeObject>(mediaObj as ReferenceLike) ??
			(mediaObj as MediaTypeObject);
		...
		if (resolved.examples && typeof resolved.examples === "object") {
			...
			if (firstExample !== undefined) {
				...
				const picked = this.extractExampleValue(
					resolvedExample,
					preferSerialized,
				);
				...
				if (resolvedExample !== null && typeof resolvedExample !== "object")
	...
	private buildOperationMetaTags(operation: PathInfo): string[] {
		...
		if (rawOperation && Object.hasOwn(rawOperation, "servers")) {
			const servers = rawOperation.servers ?? [];

			tags.push(`@server ${JSON.stringify(servers)}`);
		}

		if (rawOperation && Object.hasOwn(rawOperation, "security")) {
		...
		if (querystringParam) {
			const contentType = querystringParam.content
				? Object.keys(querystringParam.content)[0]
				: undefined;
			...
			if (typeof querystringParam.required === "boolean")
				meta.required = querystringParam.required;

			if (querystringParam.description)
				meta.description = sanitizeComment(querystringParam.description);
	...
	private filterMediaTypes(mediaTypes: string[]): string[] {
		...
		mediaTypes.forEach((raw) => {
			...
			entries.push({
				raw,
				normalized,
				specificity: this.mediaTypeSpecificity(normalized),
			});
		});
	...
	private getDistinctNegotiationVariants(
		...
		const filteredMediaTypes = new Set(
			this.filterMediaTypes(uniqueByMedia.map((v) => v.mediaType)),
		);
	...
	private emitMethodBody(
		...
		const xmlParams =
			rawOp.parameters
				...
				.filter(
					(p): p is { param: Parameter; schema: SwaggerDefinition | boolean } =>
						!!p,
				) ?? [];
		...
		xmlParams.forEach(({ param, schema }) => {
			...
			const rootName =
				typeof schema === "object" && schema.xml?.name
					? schema.xml.name
					: param.name;
			const xmlConfig = (
		...
		const qsParam = rawOp.parameters?.find(
			(p: Parameter) => (p.in as string) === "querystring",
		);
		...
		if (qsParam) {
			...
			const isJson =
				qsParam.content?.["application/json"] ||
				contentType?.includes("application/json");

			const qsConfig = model.queryParams.find(
				(p) => p.originalName === qsParam.name,
			);
			...
			const contentTypeArg =
				!isJson && contentType ? `'${contentType}'` : "undefined";

			const encodingArg = encodingConfig
			...
			lines.push(
				`const queryString = ParameterSerializer.serializeRawQuerystring(${args.join(", ")});`,
			);
		...
		lines.push(
			`const url = \`\${basePath}${urlTemplate}${queryStringVariable}\`;`,
		);

		const standardQueryParams = model.queryParams.filter(
			(p) => p.originalName !== qsParam?.name,
		);

		if (standardQueryParams.length > 0) {
			lines.push(
				`let params = new HttpParams({ encoder: new ApiParameterCodec(), fromObject: (options?.params && !(options.params instanceof HttpParams) ? options.params : {}) as Record<string, string | number | boolean | readonly (string | number | boolean)[]> });`,
			);
			...
			standardQueryParams.forEach((p: ParamSerialization) => {
				...
				lines.push(
					`serialized_${p.paramName}.forEach((entry: { key: string, value: string | Blob }) => params = params.append(entry.key, entry.value as string));`,
				);
			});
		}
		...
		if (model.cookieParams.length > 0) {
			if (this.config.options.platform !== "node") {
				lines.push(
					`// WARNING: Setting 'Cookie' headers manually is forbidden in browsers.`,
				);
			...
			model.cookieParams.forEach((p: ParamSerialization) => {
				...
				const hint =
					p.serializationLink === "json"
						? ", 'json'"
						: hasEncoderConfig
							? ", undefined"
							: "";

				const encoderArg = hasEncoderConfig
					? `, ${JSON.stringify(p.contentEncoderConfig)}`
					: "";
		...
		if (hasContentNegotiation) {
			const xmlOrSeqCondition = variantsForNegotiation
				...
				.filter(
					(v) =>
						v.serialization === "xml" || v.serialization.startsWith("json-"),
				)
			...
			const binaryCondition = variantsForNegotiation
				...
				.filter(
					(v) =>
						v.serialization === "blob" || v.serialization === "arraybuffer",
				)
			...
			if (binaryCondition || xmlOrSeqCondition) {
				const binaryResponseType = variantsForNegotiation.some(
					(v) => v.serialization === "arraybuffer",
				)
					? "arraybuffer"
					: "blob";
			...
			const isSeq =
				model.responseSerialization === "json-seq" ||
				model.responseSerialization === "json-lines";
  ...
  context: ${contextConstruction}`;
		...
		lines.push(
			`let requestOptions: HttpRequestOptions = {${optionProperties}\n};`,
		);
		...
		if (legacyFormData && legacyFormData.length > 0) {
			if (isUrlEnc) {
				...
				legacyFormData.forEach((p: Parameter) => {
					...
					lines.push(
						`if (${paramName} != null) { formBody = formBody.append('${p.name}', ${paramName}); }`,
					);
				});
				...
				legacyFormData.forEach((p: Parameter) => {
					...
					lines.push(
						`if (${paramName} != null) { formData.append('${p.name}', ${paramName}); }`,
					);
				});
			...
			if (body.type === "raw" || body.type === "json") {
				...
				if (model.requestEncodingConfig) {
					lines.push(
						`if (${body.paramName} !== null && ${body.paramName} !== undefined) {`,
					);
				...
				if (model.requestEncodingConfig) {
					...
					lines.push(
						`if (encodedBody !== null && encodedBody !== undefined) {`,
					);
				...
				const bodyVar =
					body.type === "json-seq" ? "jsonSeqBody" : "jsonLinesBody";
				...
				if (body.type === "json-seq") {
					...
					lines.push(
						`  ${bodyVar} = ${bodyVar}.map((item: Record<string, string | number | boolean | object | undefined | null>) => '\\x1e' + JSON.stringify(item)).join('');`,
					);

					lines.push(
						`} else if (${bodyVar} != null && typeof ${bodyVar} !== 'string') {`,
					);
					...
					lines.push(
						`  ${bodyVar} = ${bodyVar}.map((item: Record<string, string | number | boolean | object | undefined | null>) => JSON.stringify(item)).join('\\n');`,
					);
				...
				lines.push(
					`const multipartResult = MultipartBuilder.serialize(${body.paramName}, multipartConfig);`,
				);
				...
				lines.push(
					`  requestOptions = { ...requestOptions, headers: newHeaders };`,
				);
		...
		if (isSSE) {
			...
			const hasSseDecoding =
				model.responseDecodingConfig &&
				Object.keys(model.responseDecodingConfig).length > 0;

			if (hasSseDecoding) {
				lines.push(
					`const sseDecodingConfig = ${JSON.stringify(model.responseDecodingConfig)};`,
				);
			}
            ...
            return new Observable<${model.responseType}>(observer => { 
                ...
                const fetchHeaders = (() => { 
                    if (typeof Headers !== 'undefined') { 
                        ...
                        if (headers instanceof HttpHeaders) { 
                            ...
                            Object.entries(headers as Record<string, string | number | boolean | object | undefined | null>).forEach(([key, value]) => { 
                ...
                ${bodyArgument !== "null" ? `fetchOptions.body = ${bodyArgument} as Record<string, string | number | boolean | object | undefined | null>;` : ""} 
                ...
                fetch(url, fetchOptions).then(response => { 
                    ...
                    const dispatch = () => { 
                        ...
                        let payload: Record<string, string | number | boolean | object | undefined | null>; 
                        if ('${sseMode}' === 'event') { 
                            payload = { data }; 
                        ...
                        observer.next(payload as Record<string, string | number | boolean | object | undefined | null>); 
                        resetEvent(); 
                    }; 
		...
		const isStandardBody = ["post", "put", "patch", "query"].includes(
			httpMethod,
		);

		const isStandardNonBody = [
			"get",
			"delete",
			"head",
			"options",
		...
		const returnGeneric = `string | number | boolean | object | undefined | null`;

		let httpCall: string;

		if (isStandardBody) {
			if (httpMethod === "query") {
				httpCall = `this.http.request('QUERY', url, { ...requestOptions, body: ${bodyArgument} } as Record<string, string | number | boolean | object | undefined | null>)`;
			} else {
				httpCall = `this.http.${httpMethod}<${returnGeneric}>(url, ${bodyArgument}, requestOptions as object)`;
			}
		} else if (bodyArgument !== "null") {
			httpCall = `this.http.request<${returnGeneric}>('${model.httpMethod}', url, { ...requestOptions, body: ${bodyArgument} } as Record<string, string | number | boolean | object | undefined | null>)`;
		} else if (isStandardNonBody) {
			httpCall = `this.http.${httpMethod}<${returnGeneric}>(url, requestOptions as object)`;
		} else {
			httpCall = `this.http.request<${returnGeneric}>('${model.httpMethod}', url, requestOptions as object)`;
		}
		...
		if (hasContentNegotiation) {
			...
			variantsForNegotiation.forEach((v) => {
				...
				const isSeq =
					v.serialization === "json-seq" || v.serialization === "json-lines";
				...
				if (isXml) {
					...
					lines.push(
						`       return XmlParser.parse(response, ${JSON.stringify(v.xmlConfig)});`,
					);
					...
					lines.push(
						`       return ContentDecoder.decode(response, ${JSON.stringify(v.decodingConfig)});`,
					);
			...
			if (def?.decodingConfig) {
				lines.push(`    // Default decoding`);

				lines.push(
					`    return ContentDecoder.decode(response, ${JSON.stringify(def.decodingConfig)});`,
				);
			} else {
			...
			if (isSeq) {
				const delimiter =
					model.responseSerialization === "json-seq" ? "\\x1e" : "\\n";
				...
				lines.push(
					`    return XmlParser.parse(response, ${JSON.stringify(model.responseXmlConfig)});`,
				);
	...
	private emitOverloads(
		...
		return overloads.map((o) => {
			if (
				parameters.some((p) => p.hasQuestionToken) &&
				o.parameters?.find((p) => p.name === "options")
			) {
