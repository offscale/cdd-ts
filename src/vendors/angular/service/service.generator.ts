...
export class ServiceGenerator extends AbstractServiceGenerator {
	...
	public override generateServiceFile(
	...
	protected generateImports(
		...
		if (
			operations.some(
				(op) =>
					op.consumes?.includes("multipart/form-data") ||
		...
		const schemaHasDecodingHints = (
			...
			if (Array.isArray(schema)) {
				return schema.some((item: OpenApiValue) =>
					schemaHasDecodingHints(
						item as SwaggerDefinition | boolean,
						depth - 1,
					),
				);
			}
		...
		for (const op of operations) {
			hasDecoding =
				...
				Object.values(op.responses!).some((r) => {
					...
					return Object.values(r.content).some((media) => {
						...
						return schemaHasDecodingHints(
							schema as SwaggerDefinition | boolean,
						);
					});
				});
			...
			if (op.requestBody?.content?.["application/json"]?.schema) {
				...
				hasEncoding =
					hasEncoding || JSON.stringify(s).includes("contentMediaType");
			}
			...
			if (op.parameters && op.parameters.length > 0) {
				const hasParamEncoding = op.parameters.some((param) => {
					...
					return (
						serialized.includes("contentEncoding") ||
						serialized.includes("contentMediaType")
					);
				});
		...
		const hasExtensions = operations.some((op) =>
			Object.keys(op).some((k) => k.startsWith("x-")),
		);
	...
	protected generateServiceContent(
		sourceFile: SourceFile,
		controllerName: string,
		operations: PathInfo[],
	): void {
