 ...
 */
function formatLiteralValue(val: OpenApiValue): string {
...
function pickPreferredMediaType(
	...
	const sorted = [...filtered].sort((a, b) => {
		const priorityDiff =
			getMediaTypePriority(a.normalized) - getMediaTypePriority(b.normalized);
...
export function getTypeScriptType(
	...
	if (schema.nullable) {
		...
		const baseType = getTypeScriptType(
			rest as SwaggerDefinition,
			config,
			knownTypes,
		);

		if (
			baseType === "string | number | boolean | object | undefined | null" ||
			/\bnull\b/.test(baseType)
		) {
			return baseType;
		}
		...
		if (dependentRequired) {
			Object.entries(dependentRequired).forEach(([propName, requiredList]) => {
				...
				const requiredFields = requiredList
					...
					.map((req) =>
						/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(req) ? req : `'${req}'`,
					)

					.map(
						(req) =>
							`${req}: string | number | boolean | object | undefined | null`,
					)
					.join("; ");
	...
	if (schema.enum) {
		if (schema.title && knownTypes.includes(pascalCase(schema.title)))
	...
	const hasContentEncoding =
		...
		schema.contentEncoding.length > 0;
	const isStringLike =
		schema.type === "string" ||
...
function getNumberType(
	...
	if (schema.format === "int64" && config.options.int64Type)
		return config.options.int64Type;
...
function getArrayType(
	...
	if (schema.prefixItems && Array.isArray(schema.prefixItems)) {
		const prefixTypes = schema.prefixItems.map((s) =>
			getTypeScriptType(s, config, knownTypes),
		);
		...
		if (schema.items && !Array.isArray(schema.items)) {
			...
			if (unevaluated === true) {
				...
				const innerType = getTypeScriptType(
					unevaluated as SwaggerDefinition,
					config,
					knownTypes,
				);
	...
	const itemsType = getTypeScriptType(
		itemsSchema as SwaggerDefinition | boolean,
		config,
		knownTypes,
	);

	return itemsType.includes("|") || itemsType.includes("&")
		? `(${itemsType})[]`
		: `${itemsType}[]`;
}
...
function getObjectType(
	...
	const indexSignatureTypes: string[] = [];
	...
	if (schema.additionalProperties) {
		const valueType =
			schema.additionalProperties === true
				? "string | number | boolean | object | undefined | null"
				: getTypeScriptType(
						schema.additionalProperties as SwaggerDefinition,
						config,
						knownTypes,
					);
	...
	if (
		...
		const patternTypes = Object.values(schema.patternProperties)
			...
			.map((patternSchema) =>
				getTypeScriptType(
					patternSchema as SwaggerDefinition | boolean,
					config,
					knownTypes,
				),
			)
	...
	if (schema.unevaluatedProperties) {
		const valueType =
			schema.unevaluatedProperties === true
				? "string | number | boolean | object | undefined | null"
				: getTypeScriptType(
						schema.unevaluatedProperties as SwaggerDefinition,
						config,
						knownTypes,
					);
...
export function getResponseType(
	...
	if (!response?.content) return "void";
