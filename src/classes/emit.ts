...
export class TypeGenerator {
		...
		allPaths.forEach((op) => {
			if (op.callbacks) {
				Object.entries(op.callbacks).forEach(([callbackName, callbackObj]) => {
					const resolvedCallback = this.parser.resolve(callbackObj) as Record<
						string,
						PathItem
					>;
					...
					for (const [headerName, headerObj] of Object.entries(resp.headers)) {
						...
						if (schema === undefined && resolvedHeader.content) {
							...
							if (
								...
								schema = resolvedHeader.content[firstContentType]
						...
						if (schema === undefined) {
							schema = {
								type: resolvedHeader.type,
								format: resolvedHeader.format,
							} as Record<
								string,
	...
	private shouldGenerateInterface(def: SwaggerDefinition | boolean): boolean {
		...
		if (
			...
			(def as Record<string, OpenApiValue>).dependentRequired
		)
			return false;
	...
	private generateInterface(
		...
		const responseProps = this.getInterfaceProperties(def, {
		...
		if (this.needsRequestModel(def)) {
			const requestProps = this.getInterfaceProperties(def, {
	...
	private applyIndexSignature(
		...
		if (returnTypes.length > 0) {
			...
			const returnType = distinct.includes(
				"string | number | boolean | object | undefined | null",
			)
				? "string | number | boolean | object | undefined | null"
				: distinct.join(" | ");
		...
		if (def.properties) {
			Object.entries(def.properties).forEach(([propName, propDefObj]) => {
				...
				props.push({
					...
					...(options.excludeWriteOnly && propDef.readOnly
						? { isReadonly: true }
						: {}),
				});
			});
	...
	private needsRequestModel(def: SwaggerDefinition): boolean {
		const hasDirect =
			def.properties &&
			Object.values(def.properties).some(
				(p) => typeof p === "object" && (p.readOnly || p.writeOnly),
			);
		...
		if (def.allOf) {
			return def.allOf.some((subObj) => {
				...
				if (sub.$ref) {
					...
					return resolved
						? this.needsRequestModel(resolved as SwaggerDefinition)
						: false;
				}

				return (
					sub.properties &&
					Object.values(sub.properties).some(
						(p) => typeof p === "object" && (p.readOnly || p.writeOnly),
					)
				);
			});
	...
	private buildJSDoc(
		...
		if ((def as Record<string, OpenApiValue>).deprecated)
			tags.push({ tagName: "deprecated" });

		if (def.example !== undefined) {
			tags.push({
				tagName: "example",
				text: JSON.stringify(def.example, null, 2),
			});
		}
		...
		if (def.default !== undefined)
			tags.push({ tagName: "default", text: JSON.stringify(def.default) });

		if (def.externalDocs?.url) {
			const desc = def.externalDocs.description
				? ` - ${sanitizeComment(def.externalDocs.description)}`
				: "";
		...
		pushTag(
			"propertyNames",
			(def as Record<string, OpenApiValue>).propertyNames,
		);

		if (
			Object.hasOwn(def as Record<string, OpenApiValue>, "additionalProperties")
		) {
			pushTag(
				"additionalProperties",
				(def as Record<string, OpenApiValue>).additionalProperties,
			);
		}
		...
		pushTag("nullable", (def as Record<string, OpenApiValue>).nullable, {
			omitTrue: true,
		});
		...
		pushTag(
			"schemaDynamicAnchor",
			(def as Record<string, OpenApiValue>).$dynamicAnchor,
		);
		...
		pushTag(
			"contentMediaType",
			(def as Record<string, OpenApiValue>).contentMediaType,
		);

		pushTag(
			"contentEncoding",
			(def as Record<string, OpenApiValue>).contentEncoding,
		);

		pushTag(
			"contentSchema",
			(def as Record<string, OpenApiValue>).contentSchema,
		);

		pushTag(
			"patternProperties",
			(def as Record<string, OpenApiValue>).patternProperties,
		);

		pushTag(
			"dependentSchemas",
			(def as Record<string, OpenApiValue>).dependentSchemas,
		);
		...
		pushTag(
			...
			(def as Record<string, OpenApiValue>).unevaluatedProperties,
		);

		pushTag(
			"unevaluatedItems",
			(def as Record<string, OpenApiValue>).unevaluatedItems,
		);
		...
		pushTag(
			"discriminator",
			(def as Record<string, OpenApiValue>).discriminator,
		);

		const extensionEntries = Object.entries(
			def as Record<string, OpenApiValue>,
