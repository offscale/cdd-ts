 ...
 */
export function parseGeneratedModelSource(
	sourceText: string,
 ...
 */
export function parseGeneratedModels(
	inputPath: string,
	fileSystem: ReverseModelFileSystem,
): ReverseSchemaMap {
	...
	if (stat.isFile()) {
		if (!isModelFilePath(inputPath)) {
			throw new Error(
				`Expected a generated model file (*.ts). Received: ${inputPath}`,
			);
		}
	...
	if (Object.keys(schemas).length === 0) {
		throw new Error(
			`No exported models could be reconstructed from: ${inputPath}`,
		);
	}
...
function inferDiscriminators(schemas: ReverseSchemaMap): void {
	Object.values(schemas).forEach((schemaEntry) => {
		...
		schema.discriminator = hasRefs
			? { propertyName: candidate, mapping }
		...
		if (
			variants.every((variant) => {
				...
				return (
					...
					getDiscriminatorValueSchema(propSchema as SwaggerDefinition) !==
						undefined
				);
			})
		) {
...
function getDiscriminatorValueSchema(
	schema: SwaggerDefinition,
): string | number | boolean | undefined {
	if (schema.const !== undefined)
		return schema.const as string | number | boolean;
...
function schemaFromInterface(
	...
	const ownSchema = buildObjectSchema(
		interfaceDecl.getProperties(),
		interfaceDecl.getIndexSignatures(),
	);
	...
	if (extendsTypes.length > 0) {
		const refs = extendsTypes.map((name) => ({
			$ref: `#/components/schemas/${name}`,
		}));

		if (
			Object.keys(ownSchema).length > 0 &&
			Object.keys(ownSchema.properties || {}).length > 0
		) {
			schema = { allOf: [...refs, ownSchema] };
		} else if (
			Object.keys(ownSchema).length > 0 &&
			ownSchema.additionalProperties
		) {
			schema = { allOf: [...refs, ownSchema] };
		} else {
			schema = { allOf: refs };
...
function buildObjectSchema(
	...
	if (indexSignature) {
		...
		schema.additionalProperties = returnTypeNode
			? schemaFromTypeNode(returnTypeNode)
			: {};
	}
...
export function schemaFromTypeNode(node: TypeNode): SwaggerDefinition {
	switch (node.getKind()) {
		...
		case SyntaxKind.ArrayType: {
			...
			return {
				type: "array",
				items: schemaFromTypeNode(arrayNode.getElementTypeNode()),
			};
		}
		...
		case SyntaxKind.TypeReference:
			return schemaFromTypeReference(
				node as import("ts-morph").TypeReferenceNode,
			);
		case SyntaxKind.UnionType:
			return schemaFromUnion(node as import("ts-morph").UnionTypeNode);
		case SyntaxKind.IntersectionType:
			return schemaFromIntersection(
				node as import("ts-morph").IntersectionTypeNode,
			);
		...
		case SyntaxKind.ParenthesizedType:
			return schemaFromTypeNode(
				(node as import("ts-morph").ParenthesizedTypeNode).getTypeNode(),
			);
		case SyntaxKind.TypeOperator: {
			const typeOp = node as OpenApiValue as { getTypeNode?: () => TypeNode };

			return schemaFromTypeNode(
				typeOp.getTypeNode ? typeOp.getTypeNode?.() : node,
			);
		}
		default:
			return {};
...
function schemaFromLiteral(
	node: import("ts-morph").LiteralTypeNode,
): SwaggerDefinition {
...
function schemaFromTypeReference(
	...
	if (typeName === "Blob" || typeName === "File")
		return { type: "string", format: "binary" };

	return { $ref: `#/components/schemas/${typeName}` };
}

function schemaFromTupleType(
	tupleNode: import("ts-morph").TupleTypeNode,
): SwaggerDefinition {
	...
	for (const element of elements) {
		...
		if (Node.isNamedTupleMember(element)) {
		...
		} else if (Node.isRestTypeNode(element as Node)) {
			restSchema = schemaFromTypeNode(
				(element as unknown as { getTypeNode: () => TypeNode }).getTypeNode(),
			);
			...
			const elementNode = element.compilerNode as unknown as { type?: unknown };
			if (elementNode.type) {
				typeNode = (
					element as unknown as {
						_getNodeFromCompilerNode: (node: unknown) => TypeNode;
					}
				)._getNodeFromCompilerNode(elementNode.type);
			} else {
	...
	if (prefixItems.length === 0 && restSchema) {
		if (
			typeof restSchema === "object" &&
			restSchema !== null &&
			restSchema.type === "array"
		) {
...
function schemaFromUnion(
	node: import("ts-morph").UnionTypeNode,
): SwaggerDefinition {
...
function schemaFromIntersection(
	node: import("ts-morph").IntersectionTypeNode,
): SwaggerDefinition {
...
function isNullTypeNode(node: TypeNode): boolean {
	...
	return (
		...
		node.getLiteral().getKind() === SyntaxKind.NullKeyword
	);
}
...
function isUndefinedTypeNode(node: TypeNode): boolean {
	...
	if (node.getKind() === SyntaxKind.TypeReference) {
		return (
			(node as import("ts-morph").TypeReferenceNode).getTypeName().getText() ===
			"undefined"
		);
	}
...
function applyNullability(schema: SwaggerDefinition): SwaggerDefinition {
	...
	if (schema.type) {
		const existing = Array.isArray(schema.type)
			? [...schema.type]
			: [schema.type];
...
function applyDocs(schema: SwaggerDefinition, node: Node): void {
	const nodeAsAny = node as OpenApiValue as {
	...
	for (const tag of primaryDoc.getTags()) {
		...
		switch (normalized) {
			...
			case "if": {
				if (
					text &&
					(typeof rawValue === "object" || typeof rawValue === "boolean")
				) {
