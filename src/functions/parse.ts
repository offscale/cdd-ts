...
const DEFAULT_INFO: InfoObject = {
	title: "Recovered OpenAPI",
	version: "0.0.0",
};
...
export type CodeScanParamLocation =
	| "path"
	| "query"
	| "header"
	| "cookie"
...
export function scanTypeScriptSource(
	...
	const schemas =
		options.includeSchemas === false
			? {}
			: extractSchemasFromSource(sourceText, filePath);
...
export function scanTypeScriptProject(
	...
	if (stat.isFile()) {
		if (!isSourceFilePath(resolved)) {
			throw new Error(
				`Expected a TypeScript source file (*.ts). Received: ${resolved}`,
			);
		}
		...
		throw new Error(
			`Input path is neither a file nor a directory: ${resolved}`,
		);
	}
	...
	for (const filePath of filePaths) {
		const contents = (
			fileSystem.readFileSync as (f: string, e: string) => string
		)(filePath, "utf-8");
 ...
 */
export function buildOpenApiSpecFromScan(
	ir: CodeScanIr,
	...
	for (const op of ir.operations) {
		...
		const pathItem: import("@src/core/types/index.js").PathItem =
			paths[op.path] ?? {};
		...
		if (standardMethods.has(methodKey)) {
			...
			const additional =
				(pathItem.additionalOperations as Record<string, OpenApiValue>) ?? {};
...
function scanExpressRoutes(sourceFile: SourceFile): CodeScanOperation[] {
	...
	for (const call of callExpressions) {
		...
		const operation = buildExpressOperation(
			routeInfo.method,
			routeInfo.path,
			handler,
...
function scanDecoratedControllers(sourceFile: SourceFile): CodeScanOperation[] {
	...
	for (const cls of sourceFile.getClasses()) {
		const controllerDecorator = cls
			.getDecorators()
			.find((dec) => CONTROLLER_DECORATORS.has(dec.getName()));
...
function extractRouteCall(call: CallExpression): RouteCallInfo | undefined {
	...
	const directPath = firstArg
		? extractPathFromExpression(firstArg as Expression)
		: undefined;
...
function buildExpressOperation(
	...
	const docMeta = handler
		? extractDocMeta(handler as import("ts-morph").Node)
		: {};
	...
	const analysis = handler
		...
		: {
				...
				responses: [
					{ status: "200", description: "Response", contentTypes: [] },
				],
				responseSchema: undefined,
			};
	...
	const responses = mergeResponseHints(
		...
		analysis.responseSchema !== undefined &&
		response.contentTypes.length > 0 &&
		response.schema === undefined
...
function analyzeExpressHandler(
	...
	if (inferredSchemas.responseSchema !== undefined) {
		responses.forEach((response) => {
			if (response.contentTypes.length > 0) {
				response.schema = inferredSchemas.responseSchema as
					| SwaggerDefinition
					| boolean;
			}
		});
	...
	const requestBody = bodyUsed
		? {
				required: true,
				contentTypes:
					requestContentTypes.size > 0
						? Array.from(requestContentTypes)
						: ["application/json"],
				...(inferredSchemas.requestSchema !== undefined
					? { schema: inferredSchemas.requestSchema }
					: {}),
			}
		: undefined;
	...
	return {
		...(requestBody ? { requestBody } : {}),
		responses,
		...(inferredSchemas.responseSchema !== undefined
			? { responseSchema: inferredSchemas.responseSchema }
			: {}),
	};
}
...
function getFunctionBody(handler: import("ts-morph").Node): Node | undefined {
	return (
		handler as OpenApiValue as { getBody?(): import("ts-morph").Node }
	).getBody?.();
}
...
function extractRequestAccess(
	...
	if (Node.isPropertyAccessExpression(node)) {
		...
		const nestedLocation = resolveRequestLocation(
			node.getExpression(),
			bindings,
		);
...
function extractResponseHint(
	...
	if (method === "sendStatus") {
		...
		const status =
			(arg0 ? extractLiteralText(arg0 as Expression) : undefined) ?? "200";
	...
	if (method === "json" || method === "send" || method === "end") {
		...
		if (method === "send" && !contentType) {
			...
			contentType = inferContentTypeFromSend(
				arg0 ? (arg0 as Expression) : undefined,
			);
		}
...
function extractResponseChainMeta(
	expression: Expression,
	resName: string,
	...
	while (
		...
		Node.isPropertyAccessExpression(current.getExpression())
	) {
		const propAccessExpr =
			current.getExpression() as import("ts-morph").PropertyAccessExpression;
		...
		if (method === "status") {
			...
			status =
				(arg0 ? extractLiteralText(arg0 as Expression) : undefined) ?? status;
		}
		...
		if (method === "type") {
			...
			contentType =
				(arg0 ? extractStringLiteral(arg0 as Expression) : undefined) ??
				contentType;
		}
		...
		if (method === "set" || method === "header") {
			...
			const headerName = nameArg
				? extractStringLiteral(nameArg as Expression)
				: undefined;

			if (headerName && headerName.toLowerCase() === "content-type") {
				contentType =
					(valueArg
						? extractStringLiteral(valueArg as Expression)
...
function isResponseChain(expression: Expression, resName: string): boolean {
	...
	if (
		...
		const propAccess =
			expression.getExpression() as import("ts-morph").PropertyAccessExpression;
...
function inferContentTypeFromSend(arg?: Expression): string | undefined {
	...
	if (
		...
		Node.isArrayLiteralExpression(arg)
	) {
...
function extractRequestContentType(
	call: CallExpression,
	reqName: string,
): string | undefined {
	const callee = call.getExpression();
...
function extractRequestHeaderName(
	call: CallExpression,
	reqName: string,
): string | undefined {
	const callee = call.getExpression();
...
function inferOperationId(
	...
	return (
		camelCase(`${method} ${pathValue}`) || `${method.toLowerCase()}Operation`
	);
}
	...
	for (const param of method.getParameters()) {
		...
		for (const decorator of param.getDecorators()) {
			...
			const paramName = trimQuotes(
				(arg ? extractStringLiteral(arg as Expression) : undefined) ??
					param.getName(),
			);
...
function buildRequestBodyFromDecoratorParams(
	...
	for (const param of method.getParameters()) {
		for (const decorator of param.getDecorators()) {
			if (
				decorator.getName() === "Body" ||
				decorator.getName() === "BodyParam"
			) {
...
function extractDocMeta(node: Node): {
	...
	const deprecated =
		tags.some((tag) => tag.getTagName() === "deprecated") ||
		rawComment.toLowerCase().includes("@deprecated");
	...
	if (Object.keys(responseSummaries).length > 0) {
		const responseIndex = new Map(
			responseHints.map((entry) => [entry.status, entry]),
		);
...
function extractQuerystringParam(
	tags: import("ts-morph").JSDocTag[],
): QuerystringMeta | undefined {
...
function applyQuerystringMeta(
	docMeta: { querystring?: QuerystringMeta },
	paramMap: Map<string, CodeScanParam>,
): void {
	...
	addParam(paramMap, {
		...
		...(querystring.required !== undefined
...
function mergeResponseHints(
	responses: CodeScanResponse[],
	hints?: ResponseDocMeta[],
): CodeScanResponse[] {
...
function parseParamDoc(
	tag: import("ts-morph").JSDocTag,
	...
	try {
		...
		if (Array.isArray(parsed)) {
			return parsed.filter(
				(entry: OpenApiValue): entry is Record<string, string[]> =>
...
function getJsDocs(node: Node): import("ts-morph").JSDoc[] {
	...
	for (let depth = 0; current && depth < 4; depth += 1) {
		const withDocs = current as Node & {
			getJsDocs?: () => import("ts-morph").JSDoc[];
		};
...
function normalizeTagJson(parsed: OpenApiValue): {
	names: string[];
	objects: TagObject[];
} {
	const names: string[] = [];
...
function normalizeDocComment(comment: OpenApiValue): string {
	...
	if (Array.isArray(comment))
		return comment
			.map((part: OpenApiValue) => normalizeDocComment(part))
			.join("");
...
function isExampleCarrier(value: OpenApiValue): value is ExampleCarrier {
	...
	if (!(EXAMPLE_WRAPPER_KEY in (value as Record<string, OpenApiValue>)))
		return false;
...
function normalizeParamSchemaOverride(
	value: OpenApiValue,
): SwaggerDefinition | boolean | undefined {
	if (typeof value === "boolean") return value;

	if (value && typeof value === "object" && !Array.isArray(value))
		return value as SwaggerDefinition;
...
function buildParameters(params: CodeScanParam[]): Parameter[] {
	return params
		...
		.filter(
			(param) => !(param.in === "header" && isReservedHeaderParam(param.name)),
		)
		.map((param) => {
			...
			return {
				...
				...(param.example !== undefined && unwrapExampleCarrier(param.example)
					? {
							examples: {
								example: unwrapExampleCarrier(param.example) as ExampleObject,
							},
						}
					: {}),
			} as Parameter;
...
function buildRequestBody(
	...
	if (requestBody.examples && Object.keys(requestBody.examples).length > 0) {
		Object.entries(content).forEach(([contentType, entry]) => {
			const example =
				requestBody.examples?.[contentType] ?? requestBody.examples?.["*"];

			if (example !== undefined) {
				const wrapped = unwrapExampleCarrier(example);

				if (wrapped) {
					(entry as { examples?: Record<string, ExampleObject> }).examples = {
						example: wrapped,
					};
				} else {
...
function buildResponses(
	...
	const normalized =
		responses.length > 0 ? responses : [{ status: "200", contentTypes: [] }];

	for (const response of normalized) {
		const description =
			response.description ??
			(response.status === "default" ? "Default response" : "Response");
		...
		if (response.contentTypes.length > 0) {
			...
			if (response.examples && Object.keys(response.examples).length > 0) {
				Object.entries(entry.content).forEach(([contentType, media]) => {
					const example =
						response.examples?.[contentType] ?? response.examples?.["*"];

					if (example !== undefined) {
						const wrapped = unwrapExampleCarrier(example);

						if (wrapped) {
							(media as { examples?: Record<string, ExampleObject> }).examples =
								{ example: wrapped };
						} else {
...
function inferReturnSchemaFromSignature(
	handler: Node,
): SwaggerDefinition | boolean | undefined {
...
function extractSchemaFromResponseType(
	typeNode?: TypeNode,
): SwaggerDefinition | boolean | undefined {
...
function inferSchemaFromTypeNode(
	typeNode?: TypeNode,
): SwaggerDefinition | boolean | undefined {
...
function schemaForPrimitiveReference(
	typeName: string,
...
function isVoidTypeNode(typeNode: TypeNode): boolean {
	return (
		typeNode.getKind() === SyntaxKind.VoidKeyword ||
		typeNode.getKind() === SyntaxKind.NeverKeyword
	);
}
