...
export class ServiceMethodAnalyzer {
	...
	public analyze(operation: PathInfo): ServiceMethodModel | null {
		...
		const { variants: responseVariants, successCode } = this.analyzeResponse(
			operation,
			knownTypes,
		);
		...
		const errorResponses = this.analyzeErrorResponses(
			operation,
			knownTypes,
			successCode,
		);
		...
		(operation.parameters || []).forEach((p) => {
			...
			const effectiveStyle =
				p.style || (p.in === "query" || p.in === "cookie" ? "form" : "simple");

			const defaultExplode =
				effectiveStyle === "form" || effectiveStyle === "cookie";
		...
		const requestContentType = this.getRequestBodyContentType(
			operation.requestBody,
		);

		let requestEncodingConfig: Record<string, OpenApiValue> | undefined;
		...
		if (
			...
			if (rbContent) {
				...
				if (contentType && rbContent[contentType]?.schema !== undefined) {
					const cfg = this.getEncodingConfig(
						rbContent[contentType]?.schema as SwaggerDefinition,
					);
		...
		const effectiveSecurity =
			opSecurity !== undefined ? opSecurity : specSecurity || [];
		...
		if (operation.tags && operation.tags.length > 0) {
			const tagList = operation.tags
				.map((tag) => sanitizeComment(tag))
				.filter(Boolean);
		...
		return {
			...
			...(defaultVariant.xmlConfig
				? { responseXmlConfig: defaultVariant.xmlConfig }
				: {}),
			...(defaultVariant.decodingConfig
				? { responseDecodingConfig: defaultVariant.decodingConfig }
				: {}),
			...(defaultVariant.sseMode
				? { sseMode: defaultVariant.sseMode as "event" | "data" }
				: {}),
	...
	private analyzeResponse(
			...
			if (reqSchema !== undefined) {
				variants.push({
					...
					type: getTypeScriptType(
						reqSchema as SwaggerDefinition,
						this.config,
						knownTypes,
					),
					serialization: "json",
		...
		const explicitSuccessCodes = Object.keys(responses).filter((code) =>
			/^2\d{2}$/.test(code),
		);

		const rangeSuccessCodes = Object.keys(responses).filter(
			(code) => code === "2XX",
		);

		let codesToProcess =
			explicitSuccessCodes.length > 0
				? explicitSuccessCodes
				: rangeSuccessCodes;
		...
		const addVariant = (variant: ResponseVariant) => {
			...
			const existing = variants.find(
				(v) => `${v.mediaType}|${v.type}|${v.serialization}` === key,
			);
		...
		const processResponseContent = (responseObj: SwaggerResponse) => {
			...
			Object.entries(responseObj.content).forEach(([mediaType, mediaObj]) => {
				...
				if (normalized.includes("json") || normalized === "*/*") {
					...
					let decodingConfig: Record<string, OpenApiValue> | undefined;
					...
					const sequentialSchema =
						(mediaObj as MediaTypeObject).itemSchema !== undefined
							...
							: (mediaObj as MediaTypeObject).schema !== undefined
								? ((
										(mediaObj as MediaTypeObject).schema as Record<
											string,
											OpenApiValue
										>
									)?.items ?? (mediaObj as MediaTypeObject).schema)
								: undefined;

					if (sequentialKind) {
						serialization =
							sequentialKind === "json-seq" ? "json-seq" : "json-lines";
				...
				if (normalized === "application/xml" || normalized.endsWith("+xml")) {
					const schema = (mediaObj as MediaTypeObject)
						.schema as SwaggerDefinition;
				...
				if (normalized === "text/event-stream") {
					...
					const itemType =
						effectiveSchema !== undefined
							? getTypeScriptType(
									effectiveSchema as SwaggerDefinition,
									this.config,
									knownTypes,
								)
					...
					const sseMode =
						effectiveSchema !== undefined
							? this.getSseMode(effectiveSchema as SwaggerDefinition)
							: "data";
		...
		if (variants.length > 0) {
			...
			scored.sort((a, b) => {
				const specDiff =
					this.getMediaTypeSpecificity(b.normalized) -
					this.getMediaTypeSpecificity(a.normalized);

				if (specDiff !== 0) return specDiff;

				const prefDiff =
					this.getMediaTypePreference(a.normalized) -
					this.getMediaTypePreference(b.normalized);
	...
	private analyzeErrorResponses(
		...
		for (const [code, responseObj] of Object.entries(operation.responses)) {
			...
			if ((responseObj as SwaggerResponse).content) {
				...
				const jsonSchema = pickJsonSchema(
					content as Record<string, { schema?: SwaggerDefinition }>,
				);

				if (jsonSchema !== undefined) {
					type = getTypeScriptType(
						jsonSchema as SwaggerDefinition,
						this.config,
						knownTypes,
					);
				} else {
					const xmlSchema = pickXmlSchema(
						content as Record<string, { schema?: SwaggerDefinition }>,
					);

					if (xmlSchema !== undefined) {
						type = getTypeScriptType(
							xmlSchema as SwaggerDefinition,
							this.config,
							knownTypes,
						);
					} else if (
						Object.keys(content).some((mt) =>
							this.isTextMediaType(this.normalizeMediaType(mt)),
						)
					) {
						type = "string";
					} else {
				...
				!(responseObj as SwaggerResponse).content &&
				(code === "401" || code === "403")
			) {
				type = "void";
			}
	...
	private analyzeParameters(
		...
		(operation.parameters ?? []).forEach((param) => {
			...
			if (param.content) {
				...
				if (firstType && param.content[firstType]?.schema !== undefined) {
					effectiveSchema = param.content[firstType]
						?.schema as SwaggerDefinition;
				}
			}

			const paramType = getTypeScriptType(
				effectiveSchema,
				this.config,
				knownTypes,
			);
		...
		if (requestBody) {
			...
			const normalized = contentType
				? this.normalizeMediaType(contentType)
				: undefined;
			...
			const sequentialKind = normalized
				? this.inferSequentialJsonKind(
						normalized,
						content as MediaTypeObject | undefined,
					)
				: undefined;
			...
			if (contentType && normalized && sequentialKind) {
				const itemSchema =
					(content as MediaTypeObject)?.itemSchema !== undefined
						...
						: (content as MediaTypeObject)?.schema &&
								...
								(
									(content as MediaTypeObject).schema as Record<
										string,
										OpenApiValue
									>
								).items
				...
				const itemType = itemSchema
					? getTypeScriptType(
							itemSchema as SwaggerDefinition,
							this.config,
							knownTypes,
						)
					: "string | number | boolean | object | undefined | null";
				...
				if (knownTypes.includes(rawBodyType)) {
					const schemaObj = this.parser.schemas.find(
						(s) => s.name === rawBodyType,
					);
					...
					if (
						...
						this.needsRequestType(definition)
					) {
						bodyType = bodyType.replace(rawBodyType, `${rawBodyType}Request`);
					}
				}

				const bodyName = isDataTypeInterface(rawBodyType)
					? camelCase(rawBodyType)
					: "body";
				...
				let bodyType = getTypeScriptType(
					effectiveSchema as SwaggerDefinition,
					this.config,
					knownTypes,
				);
				...
				const bodyType = rawBodyType.includes("|")
					? `(${rawBodyType})`
					: rawBodyType;
		...
		return parameters.sort(
			(a, b) => (a.hasQuestionToken ? 1 : 0) - (b.hasQuestionToken ? 1 : 0),
		);
	}
	...
	private isIgnoredHeaderParam(param: Parameter): boolean {
		...
		return (
			name === "accept" || name === "content-type" || name === "authorization"
		);
	}
	...
	private analyzeBody(
		...
		const formDataParams = operation.parameters?.filter(
			(p) => (p as Record<string, OpenApiValue>).in === "formData",
		);
		...
		if (!rb?.content) return { type: "raw", paramName: bodyParamName };
		...
		const multipartKey =
			...
			normalizedContentType === "multipart/mixed"
		...
		if (multipartKey) {
			...
			const schema = (mediaType as MediaTypeObject).schema as
				| SwaggerDefinition
				| boolean
				| undefined;
			...
			if (
				...
				if (schema.prefixItems && Array.isArray(schema.prefixItems)) {
					...
					schema.prefixItems.forEach((prefixItemSchema, index) => {
						...
						this.enrichEncodingConfig(
							prefixItemSchema as SwaggerDefinition | boolean,
							wrapper,
							"temp",
						);
					});
		...
		if (this.isFormUrlEncodedMediaType(normalizedContentType)) {
			...
			const schema = (rb.content[selectedContentType] as MediaTypeObject)
				?.schema as SwaggerDefinition | boolean | undefined;
	...
	private enrichEncodingConfig(
		...
		if (resolvedProp?.contentEncoding) {
			...
			const hasTransferHeader = Object.keys(headers).some(
				(h) => h.toLowerCase() === "content-transfer-encoding",
			);
	...
	private getParameterContent(p: Parameter): {
		contentType?: string;
		...
		const encoding = (
			p.content as Record<string, { encoding?: Record<string, OpenApiValue> }>
	...
	private getParameterSchema(
		p: Parameter,
		contentType?: string,
	): SwaggerDefinition | boolean | undefined {
		...
		const entry = (
			p.content as Record<string, { schema?: SwaggerDefinition | boolean }>
		)?.[key];
	...
	private getRequestBodyContentType(
		requestBody: PathInfo["requestBody"],
	): string | undefined {
		if (!requestBody?.content) return undefined;

		return this.selectRequestBodyContentType(
			requestBody.content as Record<string, { schema?: SwaggerDefinition }>,
		);
	}

	private selectRequestBodyContentType(
		content: Record<string, { schema?: SwaggerDefinition }>,
	): string | undefined {
	...
	private getXmlConfig(
		...
		if (resolved.type === "array" && resolved.items) {
			config.items = this.getXmlConfig(
				resolved.items as SwaggerDefinition,
				depth - 1,
			);
		}
		...
		if (resolved.properties) {
			...
			Object.entries(resolved.properties).forEach(([propName, propSchema]) => {
				const propConfig = this.getXmlConfig(
					propSchema as SwaggerDefinition | boolean,
					depth - 1,
				);

				if (Object.keys(propConfig).length > 0) {
					(config.properties as Record<string, OpenApiValue>)[propName] =
						propConfig;
				}
			});
		}
		...
		if (resolved.allOf) {
			resolved.allOf.forEach((sub) => {
				const subConfig = this.getXmlConfig(
					sub as SwaggerDefinition | boolean,
					depth - 1,
				);
	...
	private getDecodingConfig(
		...
		if (resolved.contentSchema && resolved.type === "string") {
			if (resolved.contentMediaType?.includes("xml")) {
				config.decode = "xml";

				config.xmlConfig = this.getXmlConfig(
					resolved.contentSchema as SwaggerDefinition | boolean,
					5,
				);
			} else {
				config.decode = true;
		...
		if (resolved.type === "array" && resolved.items) {
			const itemConfig = this.getDecodingConfig(
				resolved.items as SwaggerDefinition,
				depth - 1,
			);
		...
		if (resolved.properties) {
			...
			Object.entries(resolved.properties).forEach(([propName, propSchema]) => {
				const pConfig = this.getDecodingConfig(
					propSchema as SwaggerDefinition | boolean,
					depth - 1,
				);
		...
		if (resolved.allOf) {
			resolved.allOf.forEach((sub) => {
				const subConfig = this.getDecodingConfig(
					sub as SwaggerDefinition | boolean,
					depth - 1,
				);
	...
	private getSseMode(
		...
		const hasDataProperty = (
			...
			if (Array.isArray(allOf)) {
				if (
					allOf.some((sub) =>
						hasDataProperty(sub as SwaggerDefinition | boolean, depth - 1),
					)
				)
					return true;
			}
		...
		return hasDataProperty(resolved as SwaggerDefinition | boolean)
			? "event"
			: "data";
	}
	...
	private getEncodingConfig(
		...
		if (
			...
			resolved.contentMediaType &&
			resolved.contentMediaType.includes("json")
		) {
			config.encode = true;
		...
		if (resolved.type === "array" && resolved.items) {
			const itemConfig = this.getEncodingConfig(
				resolved.items as SwaggerDefinition,
				depth - 1,
			);
		...
		if (resolved.properties) {
			...
			Object.entries(resolved.properties).forEach(([propName, propSchema]) => {
				const pConfig = this.getEncodingConfig(
					propSchema as SwaggerDefinition | boolean,
					depth - 1,
				);
		...
		if (resolved.allOf) {
			resolved.allOf.forEach((sub) => {
				const subConfig = this.getEncodingConfig(
					sub as SwaggerDefinition | boolean,
					depth - 1,
				);
