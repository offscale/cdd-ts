...
export class ContentEncoder {
	private static stringToBytes(input: string): Uint8Array {
		...
		if (
			typeof (
				globalThis as string | number | boolean | object | undefined | null
			).Buffer !== "undefined"
		) {
	...
	private static applyContentEncoding(
		...
		if (value instanceof Uint8Array) {
			...
			bytes = ContentEncoder.stringToBytes(value);
		} else {
			return value;
		}

		let encoded = ContentEncoder.bytesToBase64(bytes);
		if (urlSafe) {
			encoded = encoded
				.replace(/\+/g, "-")
				.replace(/\//g, "_")
	...
	public static encode(
		...
		if (config.contentEncoding) {
			const encoded = ContentEncoder.applyContentEncoding(
				current,
		...
		if (Array.isArray(current) && config.items) {
			return current.map((item) => ContentEncoder.encode(item, config.items));
		}
			...
			if (config.properties) {
				...
				Object.keys(config.properties).forEach((key) => {
					if (Object.hasOwn(current, key)) {
						result[key] = ContentEncoder.encode(
							(
