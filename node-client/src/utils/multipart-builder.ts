...
export class MultipartBuilder {
	public static serialize(
		...
		configInput: MultipartConfig | Record<string, EncodingConfig> = {},
	): MultipartResult {
		...
		const isConfigObject =
			configInput &&
			("encoding" in configInput ||
				"prefixEncoding" in configInput ||
				"itemEncoding" in configInput ||
				"mediaType" in configInput);
		...
		if (Array.isArray(body)) {
			...
			return MultipartBuilder.serializeArrayManual(body, config);
		}
		...
		const requiresManual =
			!!config.mediaType ||
			Object.values(encodingMap).some(
				(c) =>
		...
		if (requiresManual) {
			return MultipartBuilder.serializeObjectManual(body, config);
		}

		return MultipartBuilder.serializeNative(body, encodingMap);
	}

	private static serializeNative(
		body: string | number | boolean | object | undefined | null,
		encodings: Record<string, EncodingConfig>,
	): MultipartResult {
		...
		Object.entries(body).forEach(([key, value]) => {
			...
			if (Array.isArray(value)) {
				value.forEach((v) =>
					MultipartBuilder.appendFormData(formData, key, v, contentType),
				);
			} else {
				MultipartBuilder.appendFormData(formData, key, value, contentType);
			}
		});
	...
	private static serializeObjectManual(
		body: string | number | boolean | object | undefined | null,
		config: MultipartConfig,
	): MultipartResult {
		const boundary = MultipartBuilder.generateBoundary();
		...
		const serializeArrayValue = (
			items: Array<string | number | boolean | object | undefined | null>,
			style: string,
			...
			if (hasSerializationHints) {
				...
				if (typeof value === "object" && value !== null) {
					if (explode) {
						...
						const serialized = serializeObjectValue(
							value as Record<
								string,
								string | number | boolean | object | undefined | null
							>,
	...
	private static serializeArrayManual(
		body: Array<string | number | boolean | object | undefined | null>,
		config: MultipartConfig,
	): MultipartResult {
		const boundary = MultipartBuilder.generateBoundary();
		const parts: Array<string | number | boolean | object | undefined | null> =
		...
		(body as (string | number | boolean | object | null)[]).forEach(
			(item, index) => {
	...
	private static appendPart(
		...
		if (!partContentType) {
			if (value instanceof Blob)
				partContentType = value.type || "application/octet-stream";
		...
		if (partContentType?.includes("multipart")) {
			...
			const _nestedConfig = config.encoding
			...
			const nestedResult = Array.isArray(value)
				? MultipartBuilder.serializeArrayManual(
						value,
						config as MultipartConfig,
					)
				: MultipartBuilder.serializeObjectManual(
						value,
						config.encoding ? { encoding: config.encoding } : {},
					);

			payload = nestedResult.content as
				| string
				| number
				| boolean
				| object
			...
			if (nestedResult.headers?.["Content-Type"]) {
				const boundaryMatch =
					nestedResult.headers["Content-Type"].match(/boundary=(.+)$/);
		...
		parts.push(
			payload as string | number | boolean | object | undefined | null,
		);
		parts.push(crlf);
	}
